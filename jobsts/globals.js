 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"6B813861-AC7E-4910-A2B4-D0646C95B121",variableType:8}
 */
var mobBundleWeight = 0.0;
/**
 * @type {Number}
 *
 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"B3A65160-CCB8-4238-ACDB-DDC1B7CDACEB",variableType:8}
 */
var mobItemWeight = 0.0;
/**
 * @type {Number}
 *
...
/**
 * @properties={typeid:35,uuid:"E6CAE9A8-A5F5-49E4-A9D9-2D1C31B925B0",variableType:-4}
 */
var rfFocusLost = false;
/**
 * @properties={typeid:35,uuid:"5FBF1879-7EDC-45E2-92BF-BF79A7FAD75A",variableType:-4}
 */
var useISODate = false;
/**
 * @AllowToRunInFind
 * 
 * @param assocID
...
function getAssociation(assocID){
	if (aMobAssocs.length == 0){
		...
		e.where.add(e.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function getMinutesDuration(start,finish){
	...
	var endMs = Date.parse(finish);//bundling total seconds is the total seconds divided by number of idfiles and total Worker count
	var totalSeconds = Math.floor((endMs - startMs)/1000);
	//var workerCount = (mob.workers != '') ? mob.workers.split(',').length : 1;
	var pieceCount = mob.idfiles.length;
	totalSeconds = totalSeconds/pieceCount;//(workerCount*pieceCount);//total seconds update to reflect workers and pieces 20210104 should never use workers as per PP
...
function getAssociations(){
	...
	q.and
		...
		.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
	);
	/** @type {JSFoundSet<db:/stsservoy/associations>} */
...
function getStatusDescriptions(){
	...
	q.where.add(q.columns.association_id.eq(makeUUID(session.associationId)));//was missing assoc during invoc for third party
...
function rfF8ReversalTransaction(){
	...
	if (fTransEnd || (!fTransStart && !fTransEnd)){ // error message with end timed labor percentage = 100 // 20191023 and no trans end or begin
		...
		if (fResetTimed){
			...
			fsUpdater.setColumn('quantity',0);
...
function rfF8ReversalPrep(){
	...
	while (rec = transactions.getRecord(idx)){
		...
		if (rec.trans_status == mob.statusCode){// find START status code
			...
			mob.completeStatus = (mob.completeStatus*1 != 1 && rec.labor_percentage >= 100) ? 1 : 0;
...
function getLoggedEmployee(userId){
	...
	u.where.add(u.columns.user_uuid.eq(makeUUID(userId)));
	...
	e.where.add(e.columns.employee_id.eq(makeUUID(userRec.employee_id)));
...
function getWorkers(){
	...
	q1.where.add(q1.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function rfFunctionKeys(screen){
	...
	var dex = 0;
	///var fKey = "";
	//var shorts = ['F1','F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12'];
	//for (var indexs = 0;indexs < shorts.length;indexs++){
	//	plugins.window.removeShortcut(shorts[indexs]);
	//}
	...
	var noF8 = (scopes.prefs.globalF8Off*1 == 1);
	var noF8Ship = (scopes.prefs.shipF8Off*1 == 1);
...
function rfGetBarcodeIdfiles(){
	var isBundle = barcodeIsBundle(mob.barcodeId);
	mob.idfile = {};
	mob.piecemark = {};
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var q = databaseManager.createSelect('db:/stsservoy/idfiles');
	...
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	if (isBundle){
		q.where.add(q.columns.bundle_bc.eq(mob.barcodeId));
	} else {
		q.where.add(q.columns.id_serial_number_id.eq(makeUUID(mob.barcodeId)));
	}
	...
	while (index <= resultQ.getSize()){
		var rec = resultQ.getRecord(index);
		mob.idfiles.push(rec.idfile_id.toString());//changed for mssql sql server
		index++;
	}
	null;
...
function rfGetLocationStats(sLocation){
	...
	r.where.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	w.where.add(w.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	s.and
		...
		.add(s.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
...
function rfGetLocationStats2(sLocation){
	...
	i.where.add(i.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	i2.where.add(i2.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	if (ds1.getMaxRowIndex() > 0){
		ds1.rowIndex = 1;
		var resultTotWeight = Math.floor(ds1.total_weight*1+0.5);//#97 is here????
	} else {
		resultTotWeight = 0.0;
	}
...
function rfErrorShow(message){
	...
	if (1 || application.getApplicationType() == APPLICATION_TYPES.WEB_CLIENT){//reset error window location for web client
	...
	if (!dualEntry){
		application.sleep(1500);//RM3 change for hanging on leaving focus from screen/app
		application.output('Version:'+application.getVersion());
	...
	application.output('after request focus');
...
function rfErrorHide(event) {
	//if (0 && rfFocusLost){rfFocusLost = false;session.errorShow = false;return}
...
function getMenuList(){
	...
	q.where.add(q.columns.user_uuid.eq(makeUUID(session.loginId)));
...
function rfGetMobPiecemark(){
	...
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.piecemark_id.eq(makeUUID(mob.idfile.piecemark_id)));
...
function rfGetPiecesScanned(piecemarkId, sLocation){
	...
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.piecemark_id.eq(makeUUID(piecemarkId)));
	/** @type {JSFoundSet<db:/stsservoy/idfiles>} */
	var resultQ = databaseManager.getFoundSet(q);
	//var countPieces = 0;
	...
	r.where.add(r.columns.status_description_id.eq(makeUUID(endStation)));
	if (timed){
		var maxPercent = '100';
...
function onStartLoadPrefs(prefsType){
	...
	fs.where.add(
		fs.or
		 ...
		 .add(fs.columns.user_uuid.eq(makeUUID(session.loginId)))
		);
...
function rfGetTransactionList(idfileId){
	...
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.idfile_id.eq(makeUUID(mob.idfile.idfile_id)));
...
function rfTimedReset(){
	...
	mob.labor_percentage = 0;
	mob.timedCloseOut = 0;
	//mob.laborCompleted = false;//not used in this context
	mob.percent = 0;
}
/**
...
function rfTimed(event){
	 ...
	 */
	rfTimedReset();
	rfGetTransactions(); // latest first, check here
		...
		while (rec = transacts.getRecord(index++)){
			...
			if (rec.trans_status == mob.timedBegStat){
				if (rec.transaction_end == null){// && mob.completeAsk){ // ok to end transaction
					...
					if (!setEndDate){
						setEndDate = true;
						var endDateRevisit = new Date(rec.transaction_start);
						endDate = new Date(endDateRevisit.getTime()+mob.timedDuration*60*1000);//add minutes which is 60,000 ms times minutes
					}
					mob.timedEnd = endDate;
...
function rfLatestTransaction(singleIdfileId){
	...
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.idfile_id.eq(makeUUID(singleIdfileId)));
	var resultQ = databaseManager.getFoundSet(q);
	return resultQ;
}
/**
...
function rfLatestTransactionSet(status){
	...
	q.where.add(
		q.and
			.add(q.columns.delete_flag.isNull)
			.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
			.add(q.columns.idfile_id.eq(makeUUID(singleIdfileId)))
...
function rfSetIdfileTimedStatus(idfileId){
	...
	*/
	...
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.idfile_id.eq(makeUUID(idfileId)));
...
function rfSaveScanTransaction(event,routeOK, statusId, sLocation){
	...
	r.where.add(r.columns.status_description_id.eq(makeUUID(session.stationId)));
	r.sort.add(r.columns.transaction_date.desc);
...
//	var commitType = 'Save';//vs 'Delete'
	...
	qq.where.add(qq.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	for (index = 0; index < mob.idfiles.length; index++) {//reconsider batch backup here
		...
		if (routeOK){
			...
			if (mob.bundle.Id){
				newRecB.bundle_bc = mob.bundle.Id;
			}
			...
			newRecB.transaction_start = (mob.timedBegin)? mob.timedBegin : null;//date;//mob.timedBegin;
			...
			newRecB.labor_quantity = 1;//always one, but really matches the number of piecemarks on the barcode or is zero
			updateEditDate = true;
			if (processCodes.shipping.indexOf(statusShipType) != -1){
			...
			if (!mob.timedBegin){newRecB.quantity = 1;newRecB.labor_percentage = 100;}
			if (mob.timedEnd){
				...
				var workerCount = (currentWorkers.length > 1) ? currentWorkers.length : 1;
				...
				application.output('Timed Duration '+mob.timedDuration/mob.idfiles.length+' for '+mob.idfiles.length+' pieces for '+workerCount+' workers.');//STSmobile #0009
			}
			...
			if (mob.timedCloseOut*1 == 1){//if station complete OR not a station complete OR save as complete, save labor
				newRecB.quantity = (!mob.laborCompleted) ? 0 : 1;//totalIdfileCnt;// || mob.percent != 100
				newRecB.labor_quantity = 1;//totalIdfileCnt;
				updateEditDate = true;
			}
		...
		if (updateEditDate){newRecB.edit_date = transDate}
	}
...
function showHelp(event){
	//j4 if (forms.rf_mobile_view){forms['rf_mobile_view'].debugCount = 'YYY';return}
	application.output('entered showHelp F1 Button Dies')
	...
	application.output('right after vis')
	...
	application.output('establish visibility');
	if (!vis){
		if (form.elements['genericin']){
			form.elements['genericin'].requestFocus();
		}
		application.output('set function keyEntered false');
	...
	application.output('before rffuctionkeys');
...
function errorDialogMobile(event,errorNum,returnField,additionalMsg){
	...
	if (mobile && returnField != 'dialog'){
		...
		if (errorNum.match(/^[0-9]+$/) &&  returnField != 'info'){
	...
	//if (0 && !rfFocusLost){
		application.updateUI();
	//}
	application.output('Error dialog. After updateUI');
...
function rfProcessBarcode(event){
	...
	if (currentPercent == 100 && !flagF8 && !isInterimLoad 
			&& session.program != i18n.getI18NMessage('sts.mobile.status') 
			&& session.program != i18n.getI18NMessage('sts.mobile.build.bundles')
			&& session.program != i18n.getI18NMessage('sts.mobile.find.piece.marks')){//always allow interim load
	...
	if (!globals.laborScreenActive && !flagF8 && currentPercent > 0 && !isInterimLoad
			&& session.program != i18n.getI18NMessage('sts.mobile.build.bundles')){//skip error check to enter F8 reversal code
			...
			switch (session.program){
				...
				case i18n.getI18NMessage('sts.mobile.saw')://Saw
					...
					if (flagF8){//F8 must unload and unship 2019-10-10 08.29 Shipping Definitive Servoy Meeting.mp4
						...
						qq.where.add(qq.columns.status_code.eq(mob.statusCode));
						qq.where.add(qq.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
						qq.where.add(qq.columns.association_id.eq(makeUUID(session.associationId)));
						var QQ = databaseManager.getFoundSet(qq);
						/** @type {JSFoundSet<db:/stsservoy/status_description>} */
						var qqRec = QQ.getRecord(1);
						var statusShipType = qqRec.status_type;
						...
						if (!rfSaveThirdParties(unCommitAction)){
						...
						if (shipAction && pushStation){//push actions are for shipping activities
							application.output('return due to pushstation and shipaction');
							//return true;//20210122 was not removing entries from STSX side after pushstation null
						
						unCommitAction = 'Delete';
						var isTimed = (!(!m.stationsTimed[session.stationId]) || !(!m.stationsTimedEnds[session.stationId]));
						//20210122 add pushStation to allow this action and then save the STSX transaction changes
					...
					if (!(!session.loadId)){loadGetData();}//20181003 need to do twice. 
					break;
...
function rfWindowLastInfoDisplay(event){
	...
	if (session.program == i18n.getI18NMessage('sts.mobile.status')){
		/** @type {QBSelect<db:/stsservoy/loads>} */
		var q = databaseManager.createSelect('db:/stsservoy/loads');
		q.where.add(q.columns.load_id.eq(makeUUID(mob.idfile.current_load_id)));
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		var QQ = databaseManager.getFoundSet(q);
		if (QQ.getSize() == 1){
			/** @type {JSRecord<db:/stsservoy/loads>} */
			var recQ = QQ.getRecord(1);
			mobLoadNumber = recQ.load_number;
		} else {
			mobLoadNumber = '';
		}
	}
...
function rfValidLastMaxStatus(idfileId){
	...
	q.and
		.add(q.columns.delete_flag.isNull)
		.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
		.add(q.columns.idfile_id.eq(makeUUID(idfileId)))
	);
	/** @type {JSFoundSet<db:/stsservoy/transactions>} */
	var resultQ = databaseManager.getFoundSet(q);
...
function rfJobCheck(jobNumber,associationId){
	...
	if (associationId){
		q.where.add(q.columns.association_id.eq(makeUUID(associationId)));
	}
	var resultQ = databaseManager.getFoundSet(q);
	var size = resultQ.getSize();
	if (size == 0){
...
function rfSaveTransaction(event){
	...
	for (index = 0; index < mob.idfiles.length; index++) {
		...
		if (routeOK){
			...
			if (mob.timedBegin != '' || mob.timedEnd != ''){
			...
			transFS.labor_quantity = 1;//matches number of bardoes, since each idfile is a single instance, this always = 1 20210105
			if (event.getFormName() == 'rf_bundles'){
				transFS.bundle_bc = mob.bundle.Id;
			}
			if (application.isInDeveloper()){application.output('timed b:'+mob.timedBegin+ ' e:'+mob.timedEnd)}
			if (mob.timedBegin == '' && mob.timedEnd == ''){//untimed transactions get these settings 20210105
				transFS.quantity = 1;
				//if (transFS.trans_code == )
				transFS.labor_percentage = 100;
			}
			if (mob.timedEnd != ""){
...
function bundleCheckIdInsideElsewhere(){
	...
	q.where.add(
		q.and
			...
			.add(q.columns.idfile_id.isin(mob.idfiles))
			.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
			.add(q.columns.bundle_bc.not.isin(isEmpty))
		);
	var resultQ = databaseManager.getFoundSet(q);
	if (resultQ.getSize() > 0){
		return true;
...
function bundleGetTransactions(){
	...
	q.where.add(q.columns.status_description_id.eq(makeUUID(session.stationId)));
...
function bundleDebundle(debundleFS){
	...
	transSet.setColumn('delete_flag',10);
...
function rfGetSpecsBundle(){
	...
	while (rec = mob.bundleFS.getRecord(index++)){
		...
		if (!pcmkSeen[rec.piecemark_id]){//sorting foundset may work better, later
			...
			var q = databaseManager.createSelect('db:/stsservoy/piecemarks');
			q.result.add(q.columns.piecemark_id);
			q.where.add(q.columns.piecemark_id.eq(makeUUID(rec.piecemark_id)));
			var Q = databaseManager.getFoundSet(q);
			pmRec = Q.getRecord(1);
		}
...
function loadGetData(){
	...
	q.where.add(
		...
		q.and
			.add(q.columns.delete_flag.isNull)
			.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
			.add(q.columns.current_load_id.eq(makeUUID(session.loadId)))
		);
	}
...
function convertLoadToId(itemCSV,arrayToStr){
	if (itemCSV == ""){return null}
	//itemCSV = itemCSV.replace(/ +/g,'');//removed as if the user wants embedded spaces, then okay 20210119
	var arrayN = [];
...
function onDataChangePiecemark(oldValue, newValue, event) {
	...
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.job_id.eq(makeUUID(session.jobId)));
	...
	r.where.add(
		r.and
			...
			.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
	...
	switch (baseForm){
		case 'rf_mobile'://STSmobile // was rf_mobile_view, cause of stripping _view from end of formName
			...
			trans.where.add(trans.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function onDataChangeSequence(oldValue, newValue, event) {
	...
	s.where.add(
		s.and
			.add(s.columns.delete_flag.isNull)
			.add(s.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
			.add(s.columns.job_id.eq(makeUUID(mob.job.Id)))
...
function rfGetSpecsLoad(loadType){
	//application.output('rfGetSpecsLoad '+loadType+' No session.loadId Specified during FS call. Skipping. Tenant:'+session.tenant_uuid);
	if (!session.loadId){return;}
	var index = 1;
	var count = 0;
	var weight = 0.0;
	...
	if (typeLoad == 'ship'){
		q.where.add(q.columns.delete_flag.isNull);
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		q.where.add(q.columns.ship_load_id.eq(makeUUID(session.loadId)));
	} else if (typeLoad == 'receive'){
		q.where.add(q.columns.delete_flag.isNull);
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		q.where.add(q.columns.received_load_id.eq(makeUUID(session.loadId)));//was toString()
	} else {
		q.where.add(q.columns.delete_flag.isNull);
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		q.where.add(q.columns.current_load_id.eq(makeUUID(session.loadId)));//was toString()
	}
	...
	l.where.add(l.columns.load_id.eq(makeUUID(session.loadId)));
	l.where.add(l.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function rfGetJobIdfileIds(){
	...
	q.and
		.add(q.columns.delete_flag.isNull)
		.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
		.add(q.columns.job_id.eq(makeUUID(session.jobId)))
	);
	/** @type {JSFoundSet<db:/stsservoy/sheets>} */
	var fs = databaseManager.getFoundSet(q);
	for (var index = 1;index <= fs.getSize();index++){
...
function getCustomersByJob(){
	...
	j.where.add(j.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	if (J.getSize() > 0){
		...
		c.where.add(c.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function getJobsByAssoc(){
	...
	j.where.add(j.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	j.where.add(j.columns.delete_flag.isNull);
	j.where.add(j.columns.association_id.eq(makeUUID(session.associationId)));
...
function getJobData(custId,jobNum){
	...
	fs.where.add(fs.columns.delete_flag.isNull);
	if (custId){
		fs.where.add(fs.columns.customer_id.eq(makeUUID(custId)));
	}
...
function getJobIdSheets(){
	...
	fs.where.add(fs.columns.job_id.eq(makeUUID(jobId)));
	fs.where.add(fs.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	fs.where.add(fs.columns.delete_flag.isNull);
...
function getSequenceNumbers(){
	...
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)))
	if (!(!pcmkId)){
		q.where.add(q.columns.piecemark_id.eq(makeUUID(pcmkId)));
	}
	/** @type {JSFoundSet<db:/stsservoy/idfiles>} */
	var resultQ = databaseManager.getFoundSet(q);
	var sequenceIds = [];
...
function setUserFormPermissions(event,readOnlyForm){
	...
	var formName2 = '';
	while (formName2 = formNames1.pop()){//first set globals
		...
		while (rec = Q.getRecord(idx++)){
			...
			if (typeof forms[formName2].elements[rec.element_name].readOnly !== 'undefined'){
				forms[formName2].elements[elName].readOnly = (elEnabled != 1);
			}
			if (typeof forms[formName2].elements[rec.element_name].enabled !== 'undefined'){
	...
	while (formName2 = formNames.pop()){//now set division permissions
		...
		r.where.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		r.where.add(r.columns.user_uuid.eq(makeUUID(session.userId)));
		...
		for (var idx2 = 1;idx2 <= maxIdx;idx2++){
			...
			if (typeof forms[formName2].elements[R.element_name].readOnly !== 'undefined'){
				forms[formName2].elements[R.element_name].readOnly = (R.is_accessible != 1);
			}
			if (typeof forms[formName2].elements[R.element_name].enabled !== 'undefined'){
...
function convertFabShopToStatusDescripId(itemCSV,arrayToStr){
	...
	fs.where.add(fs.columns.association_id.eq(makeUUID(session.associationId)));
...
function onTabChangeSplit(previousIndex, event) {
	...
	prefs.where.add(prefs.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function onDataChangeAssocType(oldValue, newValue, event) {
	...
	a.where.add(a.columns.association_uuid.eq(makeUUID(forms[formName].association_uuid)));
	a.where.add(a.columns.delete_flag.not.eq(99));
	a.where.add(a.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	aa.where.add(aa.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	if (((!existing) || (existing && AA.getSize() > 1)) &&
		...
		u.where.add(u.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		u.where.add(u.columns.association_uuid.eq(makeUUID(selectedAssoc)));
		...
		if (U.getSize() == 0){
			...
			j.result.add(j.columns.job_id);
			j.where.add(j.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
			j.where.add(j.columns.association_id.eq(makeUUID(selectedAssoc)));
			var J = databaseManager.getFoundSet(j);
			J.loadRecords();
			if (J.getSize() == 0){ // check for status use of association
				...
				s.where.add(s.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
				s.where.add(s.columns.association_id.eq(makeUUID(selectedAssoc)));
				var S = databaseManager.getFoundSet(s);
				S.loadRecords();
				if (S.getSize() == 0){return true}
			}
...
function isOfficeFunction(event){
	...
	aa.result.add(aa.columns.association_uuid);
	aa.where.add(aa.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	aa.where.add(aa.columns.association_uuid.eq(makeUUID(session.associationId)));
...
function getAssociationName(assocID){
	...
	assocs.where.add(assocs.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	assocs.where.add(assocs.columns.delete_flag.isNull);
	assocs.where.add(assocs.columns.association_uuid.eq(makeUUID(theForm.vJobRec.association_id)));
...
function checkAssociationsEmpty(assocId){
	...
	as.where.add(as.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	as.where.add(as.columns.association_uuid.eq(makeUUID(assocId)));
	...
	as2.where.add(as2.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	jobFS.where.add(jobFS.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	jobFS.where.add(jobFS.columns.delete_flag.isNull);
	jobFS.where.add(jobFS.columns.association_id.eq(makeUUID(assocId)));//session.associationId
	...
	u.result.add(u.columns.user_uuid);
	u.where.add(u.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	u.where.add(u.columns.association_uuid.eq(makeUUID(assocId)));
	...
	s.result.add(s.columns.status_description_id);
	s.where.add(s.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	s.where.add(s.columns.association_id.eq(makeUUID(assocId)));
	s.where.add(s.columns.delete_flag.isNull);
	var S = databaseManager.getFoundSet(s);
...
function checkStatusEmpty(statusId){
	...
	s.where.add(s.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	s.where.add(s.columns.delete_flag.isNull);
	s.where.add(s.columns.end_for_status.eq(makeUUID(statusId)));
	var S = databaseManager.getFoundSet(s);
	if (S.getSize() > 0){tablesAffected += '[status_description]'}
	...
	t.where.add(t.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	t.where.add(t.columns.delete_flag.isNull);
	t.where.add(t.columns.status_description_id.eq(makeUUID(statusId)));
	...
	i.where.add(i.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	i.where.add(i.columns.delete_flag.isNull);
	i.where.add(i.columns.status_description_id.eq(makeUUID(statusId)));
	var I = databaseManager.getFoundSet(i);
	...
	j.where.add(j.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	j.where.add(j.columns.delete_flag.isNull);
	j.where.add(j.columns.status_description_id.eq(makeUUID(statusId)));
	var J = databaseManager.getFoundSet(j);
...
function checkRoutingCodesEmpty(routeId){
	...
	pm.where.add(pm.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	pm.where.add(pm.columns.e_route_code_id.eq(makeUUID(routeId)));
	...
	rd.where.add(rd.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	rd.where.add(rd.columns.e_route_code_id.eq(makeUUID(routeId)));
...
function checkEmployeeEmpty(event,record){
	...
	ee.where.add(ee.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	ee.where.add(ee.columns.delete_flag.isNull);
	ee.where.add(ee.columns.employee_id.eq(makeUUID(employeeId)));
	...
	tr.where.add(tr.columns.employee_id.eq(makeUUID(employeeId)));
	tr.where.add(tr.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function checkCarrierEmpty(carrierNum){
	...
	cr.where.add(cr.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function checkUserEmpty(userId){
	...
	us.where.add(us.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	us.where.add(us.columns.user_uuid.eq(makeUUID(userId)));
	...
	if (US.getSize() > 0){
		...
		ep.where.add(ep.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		ep.where.add(ep.columns.employee_id.eq(makeUUID(empId)));
...
function onDataChangeGeneric(oldValue, newValue, event) {
	...
	switch (action.toUpperCase()){
		...
		case 'D':
			...
			if (!checkBarcodeWithJob(event,data,idIsBundle)){
		...
		case 'G':
			...
			if (newValue.match(/(G[O]{0,1})$/) && 
				...
				scopes.jobs.getCutListDuration(session.cutlistdata,cutAll);//updates globals.session.cutDuration{startCut.endCut.minutes.hours}
				null;
...
function getJobDataFromID(event,barId,clearOrphans){
	...
	if (!isBundle){//11 select idfiles from bundle
		q.where.add(q.columns.id_serial_number.eq(barId.toUpperCase()));
	}
	...
	r.root.where.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
...
function rfCheckRequiredWorkerStatus(status){
	...
	r.where.add(r.columns.delete_flag.isNull);
	r.where.add(r.columns.association_id.eq(makeUUID(session.associationId)));
...
function rfLoadStats(){
	...
	var finalWeight = 0.0;
	var finalCount = 0;
	var verifyWeight = 0.0;
	var verifyCount = 0;
	var receiveWeight = 0.0;
	var receiveCount = 0;
	...
	qq.where.add(qq.columns.job_id.eq(makeUUID(jobId)));
	/** @type {QBJoin<db:/stsservoy/piecemarks>} */
	...
	q4.where.add(q4.columns.job_id.eq(makeUUID(jobId)));
	/** @type {QBJoin<db:/stsservoy/piecemarks>} */
	...
	q3.where.add(q3.columns.job_id.eq(makeUUID(jobId)));
	/** @type {QBJoin<db:/stsservoy/piecemarks>} */
	...
	q5.where.add(q5.columns.job_id.eq(makeUUID(jobId)));
	/** @type {QBJoin<db:/stsservoy/piecemarks>} */
	...
	s5.root.where.clear();
	s5.root.where.add(s5.columns.ship_load_id.eq(makeUUID(session.loadId)))
	...
	tt.where.add(tt.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	tt.where.add(tt.columns.load_id.eq(makeUUID(session.loadId)));
	...
	if (TT.getSize() > 0){
		...
		if (rec.ship_to && rec.interim_load == 0){
			...
			ad.where.add(ad.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
			ad.where.add(ad.columns.address_id.eq(makeUUID(rec.ship_to)));
...
function rfF7SaveLoadTrailerInfo(){
	...
	q.where.add(q.columns.job_id.eq(makeUUID(mob.job.Id)));
	var Q = databaseManager.getFoundSet(q);
	//var changed = false;
	if (Q.getSize() > 0){
		/** @type {JSFoundSet<db:/stsservoy/loads>} */
...
function setSampleBarsContext(){
	var serverURL = plugins.VelocityReport.getServerURL();
	var serverFolder = plugins.VelocityReport.getReportFolder();
	application.output('Server: '+serverURL+' Folder: '+serverFolder);
	...
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.delete_flag.isNull);
	q.where.add(q.columns.association_id.eq(makeUUID(session.associationId)));
	var Q = databaseManager.getDataSetByQuery(q,-1);
	...
	ss.root.where.add(ss.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	for (var idd in context){
		application.output('context: '+idd+'['+context[idd]+']');
	}
...
function silentPrint(filePath, preferredPrinter) {
	...
	application.output('Preferred Printer: '+preferredPrinter);
 	...
 	if (preferredPrinter) {
		...
		defText += ' END';
		application.output('default printer: '+defText);
	...
	// Check Printer Job
	if (!printerJob) {
		application.output('Printer Job Failed To Start');
		return false;
	} else {
		//try java program to print to printer
	}
	...
	try {
		...
		Packages.org.apache.pdfbox.pdmodel.PDFPageable
		printerJob.setPageable(new Packages.org.apache.pdfbox.printing.PDFPageable(printDocument))
		//printerJob.setPageable(printDocument);
		printerJob.print()
		if (!printDocument) {
			application.output('Printer Print Document Failed To Load');
			return false;
		}
		// Print Document
		...
		if (0){
			application.output('Silent Print Exception: '+exception);
			var fileLoc = plugins.file.getDefaultUploadLocation()+scopes.prefs.temppath.replace(/\.\\/,'\\');
			var dateOfFile = new Date().getTime();
			var execPrint = 'pushd '+plugins.file.getDefaultUploadLocation()+' & java -jar pdfbox-app-2.0.21.jar PrintPDF -printerName "'+preferredPrinter+'" -silentPrint '+filePath+' & popd';
			var runFileName = 'silentPrint'+dateOfFile+'.bat'
			var batchFile = fileLoc+'\\silentPrint'+dateOfFile+'.bat';
			var runFile = fileLoc+'\\silentPrint'+dateOfFile+'.bat';
			var runFile = plugins.file.createFile(batchFile);
			runFile.renameTo(runFileName);
			var openFile = plugins.file.writeTXTFile(runFile,execPrint);
			//var remoteRun = plugins.file.convertToRemoteJSFile(runFile.replace('\\\\','/').replace(/\\/g,'/'));
			plugins.file.openFile(remoteRun)
			application.output(execPrint);
			//plugins.file.openFile(runFile);
			application.executeProgramInBackground(runFileName,null,null,fileLoc);
			//application.executeProgramInBackground('java',new Array('-jar','pdfbox-app-2.0.21.jar','PrintPDF'+'-silentPrint','-printerName','"'+preferredPrinter+'"',filePath),null,plugins.file.getDefaultUploadLocation());
		}
		return false;
	}
	finally {
		if (printDocument && typeof printDocument.close === 'function'){//20201216 new 2020.9 version handles close differently?
			printDocument.close();
		}
	}
...
function silentPrintToServer(event){
	...
	application.output('incoming printer name: '+printerName);
	...
	if (printerName == i18n.getI18NMessage('sts.txt.printer.default')){
		printerName = scopes.prefs.defaultRfPrinter;//20201223
	}
	...
	if (isLoadReport){
		t.where.add(t.columns.current_load_id.eq(makeUUID(session.loadId)));
		/** @type {QBJoin<db:/stsservoy/loads>} */
   ...
   // t.result.add(t.functions.abs(0),'heaviest');
	...
	t.sort.add(v.columns.piecemark.asc);
	...
	if (success){//) && !application.isInDeveloper()){\\TEMP DISABLE DEVELOPER PRINT
		...
		application.output('Selected Printer Name: '+printerName);
...
function backupDatabase(event){
	...
	application.output('Backup Prog: '+backupProg);
	...
	if (0 && application.isInDeveloper()){
	...
	if (utils.stringRight(progDest,1) == '\\'){
		var runProg = progDest+''+backupProg;
	} else {
		runProg = progDest+'\\'+backupProg;
	}
	//runProg = runProg.replace(/\\\\/g,'////').replace(/\\/g,'/');
	application.output('runProg: '+runProg);
	var file = plugins.file.convertToJSFile(runProg);
	if (file.exists()){
		file.deleteFile();
	}
	...
	if (newFileOk){
		...
		globals.errorDialogMobile(event,'sts.txt.crossover.nofile',null,backupProg);
	}
	application.output('Backup Prog: '+backupProg+' Backup File: '+backupFileNameAndPath+' runProg: '+runProg);
	var remoteFile = plugins.file.convertToRemoteJSFile(runProg.replace(/\\\\/,'//').replace(/\\/g,'/'));
	//application.output('Backup Prog: '+backupProg);
	...
	application.output('database backup '+dumpCmd+' to '+backupDest+'/'+' baseDir: '+baseDir);


	application.output('Before backup program execution:'+runProg);
	var output = plugins.UserManager.Server().executeProgram(runProg);

	errorDialogMobile(event,'sts.txt.backup.initiated',null,backupProg);
	...
	if (backupProg){
		file = plugins.file.convertToJSFile(backupProg);//baseDir+'\\'+backupProg);
		if (!file){
			globals.errorDialogMobile(event,'7004',null,backupProg.replace('\\\\','/').replace(/\\/g,'/'));
			return;
		}
		backupFile = backupFile.replace(/\/\//g,'\\').replace(/\/\/\/\//g,'\\').replace(/\//g,'\\');
		application.output('backup file: '+backupFile);
		var bakFile = plugins.file.convertToJSFile(backupFileNameAndPath);
		application.output('File Spec: '+bakFile+' Path:'+backupFileNameAndPath);

		var size = bakFile.size();
		...
		if (bakFile.size() == 0){
			globals.errorDialogMobile(event,'7004',null,backupFile.replace(/\\\\/,'/').replace(/\\/g,'/'));//sts.txt.crossover.nofile
		}
	}	
}
/**
...
function rfSaveLoadTransaction(event){
	...
	while (rec = fs.getRecord(idx++)){
		...
		var q = databaseManager.createSelect('db:/stsservoy/transactions');
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		q.where.add(q.columns.idfile_id.eq(makeUUID(rec.idfile_id)));
		q.result.add(q.columns.trans_id);
		q.where.add(q.columns.status_description_id.eq(makeUUID(session.stationId)));
		/** @type {JSFoundSet<db:/stsservoy/transactions>} */
		var Q = databaseManager.getFoundSet(q);
		if (Q.getSize() == 0){
			...
			rec2.labor_percentage = 100;//20210107 added for clean data normalization
			rec2.labor_quantity = rec.summed_quantity;//20210107 added for clean data normalization
...
function getCustomerAddressNames(){
	/** @type {QBSelect<db:/stsservoy/jobs>} */
	var j = databaseManager.createSelect('db:/stsservoy/jobs');
	j.where.add(j.columns.job_id.eq(makeUUID(mob.job.Id)));
	j.where.add(j.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	q.where.add(q.columns.customer_id.eq(makeUUID(mob.customer.Id)));
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.delete_flag.isNull);
...
function clearCutListData(event){
	...
	if (typeof (globals.session.cutDuration) !== 'undefined'){//duration 664
		var durs = globals.session.cutDuration;
		durs.hours = 0.0;
		durs.startCut = null;
		durs.endCut = null;
		durs.minutes = 0.0;
...
function rfGetLoadStats(loadId){
	...
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var q = databaseManager.createSelect('db:/stsservoy/idfiles');
	q.where.add(q.columns.current_load_id.eq(makeUUID(loadId)));
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	...
	n.where.add(n.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	n.where.add(n.columns.load_id.eq(makeUUID(loadId)));
...
function rfGetLoadNumber(loadId){
	/** @type {QBSelect<db:/stsservoy/loads>} */
	var q = databaseManager.createSelect('db:/stsservoy/loads');
	q.where.add(q.columns.load_id.eq(makeUUID(loadId)));
	q.result.add(q.columns.load_number);
	var loadNum = '';
	var Q = databaseManager.getDataSetByQuery(q,-1);
	if (Q.getMaxRowIndex() > 0){
...
function onActionSelectHighlighted(event){
	...
	//scopes.jobs.updatePrintInventory(event,highlighted);
	databaseManager.saveData(fs);
}
/**
 ...
 */
function checkBarcodeWithJob(event,prodBarcode,isBundle){
	var form = forms['rf_mobile_view'];
	...
	if (isBundle){
		...
		r.where.add(r.columns.bundle_bc.eq(prodBarcode));
		r.where.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		Q = databaseManager.getFoundSet(r);
		if (Q.getSize() != 0){
			/** @type {JSRecord<db:/stsservoy/idfiles>} */
			var rec = Q.getRecord(1);
			if (rec.job_uuid.toString() == jobId.toString()){
				return true;
			}
		}
		return false;
...
function crossoverSTSdata(event){
	...
	// perform backup
	if (!application.isInDeveloper()){backupDatabase(event);}//RMM
	if (application.isInDeveloper()){
...
function rfUnsetEndTransaction(event){
	...
	var q = databaseManager.createSelect('db:/stsservoy/transactions');
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.status_description_id.eq(makeUUID(mob.begTimedStation)));
 ...
 * @param uuid
 *
 * @properties={typeid:24,uuid:"CBAD3C29-B235-4B4D-96A8-3B032128F9F2"}
 */
function makeUUID(uuid){
	if (!uuid){return null}
	if (0 && uuid.toString.length != 36){
		return uuid;
	}
 ...
 * @param idfiles
 *
 * @properties={typeid:24,uuid:"EAE53670-66E6-4580-AB2B-6EFB3D58E9B4"}
 */
function rfAddToSequencesAndLots(){
	var form = forms['rf_mobile_view'];
	if (!form.vSequenceList){form.vSequenceList = []}
	if (!form.vLotList){form.vLotList = []}
	var S = form.vSequenceList;
	var L = form.vLotList
		/** @type {QBSelect<db:/stsservoy/sequences2>} */
		var q = databaseManager.createSelect('db:/stsservoy/sequences2');
		q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		...
		var rec = null;var index = 1;
		while (rec = Q.getRecord(index++)){
			if (!S[rec.sequence_id]){S[rec.sequence_id]=rec.sequence_number;}
		}
		/** @type {QBSelect<db:/stsservoy/lots>} */
		var r = databaseManager.createSelect('db:/stsservoy/lots');
		r.where.add(r.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
		...
		/** @type {JSRecord<db:/stsservoy/lots>} */
		var rec2 = null;index = 1;
		while (rec2 = R.getRecord(index++)){
			if (!L[rec2.lot_id]){L[rec2.lot_id]=rec2.lot_number;}
		}		
}
/**
 * @param {JSEvent} event
 *
 * @properties={typeid:24,uuid:"24C624B7-F10A-40E0-B49E-29D034E0CAC7"}
 */
function checkBarTemplateWithServerOkay(event){
	/** @type {{useServerPrinters:Number,useBarTender:Number,labelName:String}} */
	var form = forms[event.getFormName()];
	var useBarTender = (form.useBarTender == 1);
	if (!useBarTender){return true}
	/** @type {String} */
	var templateName = form.labelName;
	var printerName = form.printerName;
	//if (application.isInDeveloper()){scopes.prefs.bartenderCheckDatabaseOrNot(event,templateName,printerName);}
	var useServer = (form.useServerPrinters == 1);
	var match = null;var matchPattern = new RegExp(i18n.getI18NMessage('sts.txt.use.server.side.pattern')+'\.[BbTtWw]');
	match = templateName.match(matchPattern);
	var whichDb = 'skip';
	if (globals.verifyBTTemplate){
		whichDb = scopes.prefs.bartenderCheckDatabaseOrNot(event,templateName,printerName);
	}
	if (useServer){
		if (globals.verifyBTTemplate && (whichDb == '' || whichDb != 'server')){
			globals.errorDialogMobile(event,'1305',null,'');
			return false;
		}
		if (!match){
			globals.errorDialogMobile(event,'1302',null,"'"+i18n.getI18NMessage('sts.txt.use.server.side.pattern')+"'");
			return false;
		}
	} else {
		if (globals.verifyBTTemplate && (whichDb == '' || whichDb != 'local')){
			globals.errorDialogMobile(event,'1304',null,'');
			return false;
		}
		if (match){
			globals.errorDialogMobile(event,'1303',null,"'"+i18n.getI18NMessage('sts.txt.use.server.side.pattern')+"'");
			return false;
		}
	}
	return true;
 ...
 * @param lot_uuid
 *
 * @properties={typeid:24,uuid:"4E24A746-9270-413D-9105-19902FEC4B5D"}
 */
function getLotNumber(lot_uuid){
	var form = forms['rf_mobile_view'];
	if (form && form.vLotList && form.vLotList[lot_uuid]){
		return form.vLotList[lot_uuid];
	}
	/** @type {QBSelect<db:/stsservoy/lots>} */
	var q = databaseManager.createSelect('db:/stsservoy/lots');
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.lot_id.eq(makeUUID(lot_uuid)));
	var Q = databaseManager.getFoundSet(q);
	if (Q.getSize() > 0){
		/** @type {JSRecord<db:/stsservoy/lots>} */
		var rec = Q.getRecord(1);
		if (form && form.vLotList){form.vLotList[rec.lot_id] = rec.lot_number;}
		return rec.lot_number;
	}
	return null;
}
/**
 * @param seq_uuid
 *
 * @properties={typeid:24,uuid:"9AB4E010-0036-42F4-9E1C-83B6F7F0A4AE"}
 */
function getSeqNumber(seq_uuid){
	var form = forms['rf_mobile_view'];
	if (form && form.vSequenceList && form.vSequenceList[seq_uuid]){
		return form.vSequenceList[seq_uuid];
	}
	/** @type {QBSelect<db:/stsservoy/sequences2>} */
	var q = databaseManager.createSelect('db:/stsservoy/sequences2');
	q.where.add(q.columns.tenant_uuid.eq(makeUUID(session.tenant_uuid)));
	q.where.add(q.columns.sequence_id.eq(makeUUID(seq_uuid)));
	var Q = databaseManager.getFoundSet(q);
	if (Q.getSize() > 0){
		/** @type {JSRecord<db:/stsservoy/sequences2>} */
		var rec = Q.getRecord(1);
		if (form && form.vSequenceList){form.vSequenceList[rec.sequence_id] = rec.sequence_number;}
