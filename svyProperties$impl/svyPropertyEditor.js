 ...
 * @param {String} [editorFormName]
 * 
 * @constructor 
 *
 * @properties={typeid:24,uuid:"2DBD05FC-EE43-40BE-B4E2-D3C123D64563"}
 */
function PropertyEditor(adminLevel, propertyNames, hideNonEditable, editorFormName) {
	 ...
	 * @type {String}
	 */
	var genericIconMedia = "media:///svy_component_preferences_48.png";
	 ...
	 * @type {String}
	 */
	var formName = editorFormName ? editorFormName + adminLevel : "svyPropertyEditor" + adminLevel;
	...
	this.mainForm = { 
		...
		height: 600, 
		styleClass: "main", 
		splitPaneDividerSize: 4, 
		splitPaneStyleClass: "main", 
		preventHorizontalScrollbarInPropertySets: true
	};
	...
	this.propertyValuesForm = {
		...
		offsetTop: 10,
		entrySpacing: 5,
		checkboxStyleClass: "propertyValueText",
	...
	this.createForm = function() {
		var fs = loadPropertySets(this.getPropertyNames(), this.getHideNonEditable(), this.getAdminLevel());
		...
		var mainForm = createEditor_mainForm(this, showPropertySets);
		var listForm = createEditor_propertySetForm(this);
		...
		if (showPropertySets) {
			...
			runtimeSplitPane.dividerSize = this.mainForm.splitPaneDividerSize;
			runtimeSplitPane.dividerLocation = this.propertySetsForm.width + runtimeSplitPane.dividerSize;
			if (this.mainForm.preventHorizontalScrollbarInPropertySets) {
				runtimeSplitPane.leftFormMinSize = runtimeSplitPane.dividerLocation;
			}
		}
		
		forms[mainForm.name]["propertyEditor"] = this;
		forms[mainForm.name]["adminLevel"] = adminLevel;
		
		createEditor_detailForm(null, showPropertySets, this.getFormName() + "_propertySets");		
	...
	this.showInWindow = function() {
		var runtimeForm = this.createForm();
...
function createEditor_detailForm(event, propertySetsShown, formName) {
	...
	var styleParser = new scopes.modUtils.StyleParser(styleName);
	
	var textHeight = scopes.modUtils.getTextHeight(styleParser.getFontString("label." + formSettings.labelStyleClass)) + 2;
	textHeight = Math.ceil(textHeight / 5) * 5;
	
	var headerTextHeight = scopes.modUtils.getTextHeight(styleParser.getFontString("label." + formSettings.headerStyleClass)) + 2;
	...
	var fontString = styleParser.getFontString("label." + formSettings.labelStyleClass);
	...
	for (var i = 1; i <= fs.getSize(); i++) {
		...
		for (var j = 0; j < propDescArray.length; j++) {
			...
			var textWidth = scopes.modUtils.getTextWidth(fontString, labelText) + 5;
	...
	if (labelWidth > 0 && labelWidth < ((jsForm.width - 2 * formMargin) * 0.3)) {
	...
	for (i = 1; i <= fs.getSize(); i++) {
		...
		for (j = 0; j < propDescArray.length; j++) {
			...
			if (propDesc.displayType == JSField.CHECKS) {
				...
				jsComp.styleClass = formSettings.checkboxStyleClass;
			} else {
				var jsLabel = jsForm.newLabel(labelText, formMargin, positionY, labelWidth, textHeight);
				jsLabel.styleClass = formSettings.labelStyleClass;
			...
			if (!jsComp.styleClass) {
				jsComp.styleClass = formSettings.textStyleClass;
			}
			
			if (!(propDesc.dataType == JSVariable.INTEGER || propDesc.dataType == JSVariable.NUMBER || propDesc.dataType == JSVariable.DATETIME)) {
	...
	for (var pv = 0; pv < propertyValues.length; pv++) {
		...
		jsVar = jsForm.getVariable(varName);
		if (jsVar && (jsVar.variableType == JSVariable.NUMBER || jsVar.variableType == JSVariable.INTEGER) && !(propertyValues[pv].value instanceof Number)) {
			/** @type {String} */
			var stringValue = propertyValues[pv].value;
			propertyValues[pv].value = utils.stringToNumber(stringValue);
		} else if (jsVar && jsVar.variableType == JSVariable.DATETIME && !(propertyValues[pv].value instanceof Date)) {
			/** @type {String} */
			var dateStringValue = propertyValues[pv].value;
			propertyValues[pv].value = utils.parseDate(dateStringValue, i18n.getDefaultDateFormat());	
		}
