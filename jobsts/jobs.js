 ...
 * prefixed by "_".  dsSequenceArray["_"+sequence_number+'_'+scopes.jobs.importJob.jobId] = sequence_id
 ...
 * @type {Number}
 *
 * @properties={typeid:35,uuid:"F85F4E54-3EAB-4D48-8379-CE5009F8EB17",variableType:8}
 */
 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"E4D675B6-2EEF-43E9-8697-12EA0563D830"}
 */
var prohibitedLoadPrefix = '^'
/**
...
function readLots(JobID){
	...
	q.where.add(q.columns.job_uuid.eq(globals.makeUUID(scopes.jobs.importJob.jobId)));
...
/**
 * dsSequenceArray["_"+seqNum+'_'+scopes.jobs.importJob.jobId] = sequence_id
 * dsSequenceArray[sequence_id] = "_"+"_"+seqNum+'_'+scopes.jobs.importJob.jobId
...
function readPiecemarks(jobId){
	...
	pm.where.add(pm.columns.job_uuid.eq(globals.makeUUID(jobId)));
	...
	var query = " SELECT sum(pcmk_qty) sum1, sum(item_weight*pcmk_qty) sum2 " +  // was item_quantity 20190102
	"FROM piecemarks \n" +
	"WHERE sheet_id IN " + textList + " \n"+
	" AND delete_flag IS null \n" +
	" AND tenant_uuid = '"+globals.session.tenant_uuid+"'";
	/** @type {JSDataSet} */
	var weightTotal = databaseManager.getDataSetByQuery('stsservoy', query, [] , 1);
...
function loadCurrentJobsRecords(){
	...
	for (var index2 = 1;index2 <= length2;index2++){
		...
		inSequenceId = createSequenceNumber(incoming.sequence_number);//dsSequenceArray["_"+incoming.sequence_number+'_'+importJob.jobId]; // get complete matching criteria for normal piecemark
		inLotNumber = incoming.lot_number;
		if (!inLotNumber){inLotNumber = ""}
		inLotId = createLotNumber(inLotNumber);//dsLotArray["_"+inLotNumber+'_'+importJob.jobId];
		...
		q.where.add(
			q.and
				.add(q.columns.piecemark_id.eq(globals.makeUUID(inPiecemarkId)))
				.add(q.columns.sequence_id.eq(globals.makeUUID(inSequenceId)))
				.add(q.columns.lot_id.eq(globals.makeUUID(inLotId)))
...
function tablePrefsLoad(formx){
	...
	for (var index in elems){
		...
		pp.where.add(pp.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function viewBTableSQL(criteria,formName){
	 ...
	 */
	null; //stop here 20160219 random() in posgresql rand() in mssql sql server
...
//		' left join addresses on customers.customer_id = addresses.customer_id'
	...
	if (globals.databaseName().search(/microsoft/i) != -1){
		query = query.replace(/random/i,'rand');//20210121 adjust for sql server STSALL #0015
	}
	return query;
}
/**
...
function getJobsList(){
	...
	//if (!assocOffice){
		fs.where.add(fs.columns.association_id.eq(globals.makeUUID(globals.session.associationId)));
	//}
...
function createIdfileRecord(idfilesFSet,pmkUniq,piecemarkId,sequence,lot,barcodeId,quantity,origQuant,summQuantity,remarks,bcCount,fsMajor,fsMinor){
	...
	var seqId = createSequenceNumber(sequence);//(!dsSequenceArray["_"+sequence+'_'+importJob.jobId]) ? createSequenceNumber(sequence) : dsSequenceArray["_"+sequence+'_'+importJob.jobId];
	var lotId = createLotNumber(lot);//(!dsLotArray["_"+lot]+'_'+importJob.jobId) ? createLotNumber(lot) : dsLotArray["_"+lot+'_'+importJob.jobId];
...
function createSheets(){
	...
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.job_id.eq(globals.makeUUID(importJob.jobId)));
...
function createSequenceNumber(sequenceNumber){
	...
	var unique = "_"+sequenceNumber+'_'+importJob.jobId;//20210301 RMMJOE check other calls for jobid;
	/** @type {JSFoundSet<db:/stsservoy/sequences2>} */
	var fs = databaseManager.getFoundSet('stsservoy','sequences2');
	if (dsSequenceArray[unique]){
		return dsSequenceArray[unique]
	} else {
		/** @type {QBSelect<db:/stsservoy/sequences2>} */
		var q = databaseManager.createSelect('db:/stsservoy/sequences2');
		q.where.add(q.columns.sequence_number.eq(sequenceNumber));
		q.where.add(q.columns.job_id.eq(globals.makeUUID(importJob.jobId)));
		var Q = databaseManager.getFoundSet(q);
		if (Q.getSize() != 0){
			/** @type {JSFoundSet<db:/stsservoy/sequences2>} */
			var rec2 = Q.getRecord(1);
			dsSequenceArray[unique] = rec2.sequence_id;
			dsSequenceArray[rec2.sequence_id] = unique;
			return rec2.sequence_id;
		}
	}
...
function createLotNumber(lotNumber){
	...
	if (dsLotArray[unique]){
		...
		/** @type {QBSelect<db:/stsservoy/lots>} */
		var q = databaseManager.createSelect('db:/stsservoy/lots');
		q.where.add(q.columns.lot_number.eq(lotNumber));
		q.where.add(q.columns.job_uuid.eq(globals.makeUUID(scopes.jobs.importJob.jobId)));
		var Q = databaseManager.getFoundSet(q);
		if (Q.getSize() != 0){
			/** @type {JSFoundSet<db:/stsservoy/lots>} */
			var rec2 = Q.getRecord(1);
			dsLotArray[unique] = rec2.lot_id;
			dsLotArray[rec2.lot_id] = unique;
			return rec2.lot_id;
		}
	}
	...
	rec.job_uuid = globals.makeUUID(scopes.jobs.importJob.jobId);
...
function createLotNumbers(){
	...
	for (var idx = 1;idx <= Q.getMaxRowIndex();idx++){
		...
		var unique = "_"+lot_num+'_'+scopes.jobs.importJob.jobId;//20210301
		if (dsLotArray[unique]){
...
/**	databaseManager.startTransaction();
	...
	for (index = 0;index < length;index++){*/
		var seq_num = Q.sequence_number;
		if (dsSequenceArray["_"+seq_num+'_'+importJob.jobId] != null){
...
function insertAggregPcmk(table){
	...
	for (var index = 0;index < length;index++){
		...
		sheetFS.where.add(sheetFS.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
		sheetFS.where.add(sheetFS.columns.job_id.eq(globals.makeUUID(scopes.jobs.jobIDs[0])));
		...
		pieceFS.where.add(pieceFS.columns.sheet_id.eq(globals.makeUUID(sheetID)));
		pieceFS.where.add(pieceFS.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function deleteSequencesJob(jobID){
	...
	fs.where.add(fs.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	fs.where.add(fs.columns.job_id.eq(globals.makeUUID(jobID)));
...
function purgeDeletedIdfiles(){
	...
	while (scopes.jobs.purgeBarcodeRecords.length != 0){
		...
		id.where.add(id.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
		...
		tr.where.add(tr.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function importRecordAllIdfiles(importRec){
	...
	var seqId = createSequenceNumber(seqNum);//dsSequenceArray["_"+seqNum+'_'+importJob.jobId];;
	if (lotNum){
		var lotId = createLotNumber(lotNum);//dsLotArray["_"+lotNum+'_'+importJob.jobId];
	}
...
function dbRecCounts(record){
	...
	if ((globals.session.appName == 'STS X Embedded') || (forms.kiss_option_import && forms.kiss_option_import.importOption.search('Overwrite') == -1)){
		for (var index = 0; index < tDbIdFiles.length;index++){ // get idfiles that are for this sequence and lot
			...
			if (!lotNum){lotNum = ""}
			var lotId = createLotNumber(lotNum);//dsLotArray["_"+lotNum+"|_"+seqNum];
			var seqId = createSequenceNumber(seqNum);//dsSequenceArray["_"+seqNum];
			if ((rec && rec.lprint && rec.lprint == 1) && !preserveHistory(record)){continue} // skip printed or dirty records
			if (rec.sequence_id.toString() == seqId.toString() && rec.lot_id.toString() == lotId.toString()){
...
function importRecordCheckIdfileCount(record,correct){
	...
	if ((globals.session.appName == 'STS X Embedded') || (forms.kiss_option_import && forms.kiss_option_import.importOption.search('Overwrite') == -1)){
		for (index = 0; index < tDbIdFiles.length;index++){ // get idfiles that are for this sequence and lot
			...
			if (!recLot){recLot = ""}
			var recSeqId = createSequenceNumber(recSeq);//dsSequenceArray["_"+recSeq];
			var recLotId = createLotNumber(recLot);//dsLotArray["_"+recLot+"|_"+recSeq];
	...
	*/
	...
	if (edited){
		...
		if (dsGUIDs.length == 0){
			...
			q.where.add(q.columns.import_table_id.eq(globals.makeUUID(record.import_table_id)));
...
function importRecordChangeSheet(record,unique,sheetId){
	...
	fs.where.add(fs.columns.piecemark_id.eq(globals.makeUUID(piecemarkId)));
...
function importRecords_sheet(event){
	...
	while (record = Q.getRecord(idx++)){
		...
		var seqId = createSequenceNumber(record.sequence_number);//(dsSheetArray['_'+record.sequence_number]) ? dsSheetArray['_'+record.sequence_number] : createSequenceNumber(record.sequence_number);
		var lotId = createLotNumber(record.lot_number);//(dsLotArray['_'+record.lot_number]) ? dsLotArray['_'+record.lot_number] : createLotNumber(record.lot_number);
...
function importRecordsAlt(event){
	...
	qq.where.add(qq.columns.job_id.eq(globals.makeUUID(scopes.jobs.importJobID)));
...
function getJobIdInfo(jobNum){
	if (!jobNum){
		globals.errorDialogMobile(null,1217,'genericin','');
		return null;
	}
	//databaseManager.getTableFilterParams('stsservoy');
	...
	var altTen = +'';
	application.output('jobNum: '+jobNum);//error 20210120
	jobNum = jobNum.toString().toUpperCase();
	/** @type {QBSelect<db:/stsservoy/jobs>} */
	var p = databaseManager.createSelect('db:/stsservoy/jobs');
	p.result.add(p.columns.job_id);
	p.where.add(p.columns.delete_flag.isNull);
	if (jobNum.length == 36){
		p.where.add(p.columns.job_id.eq(globals.makeUUID(jobNum)));
	} else {
		p.where.add(p.columns.job_number.eq(jobNum));
	}
	p.where.add(p.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	globals.mob.job = {Id : rec.job_id,metric : rec.metric_job,number : rec.job_number,rf : rec.rf_interface, weight : rec.job_weight};
...
function jobSheetAndDrawingNumbers(topForm){
	...
	sh.and
		.add(sh.columns.delete_flag.isNull)
		.add(sh.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
		.add(sh.columns.job_id.eq(globals.makeUUID(jobId)))
	);
	/** @type {JSFoundSet<db:/stsservoy/sheets>} */
...
function jobSequenceNumbers(topForm){
	...
	sq.and
		...
		.add(sq.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
		.add(sq.columns.job_id.eq(globals.makeUUID(jobId)))
	);
	/** @type {JSFoundSet<db:/stsservoy/sequences2>} */
...
function jobLotNumbers(topForm){
	...
	lt.and
		...
		.add(lt.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
		.add(lt.columns.job_uuid.eq(globals.makeUUID(jobId)))
	);
...
function jobLoadsAndReleases(topForm){
	...
	ld.and
		...
		.add(ld.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
		.add(ld.columns.job_id.eq(globals.makeUUID(jobId)))
	);
	...
	while (index <= fsL.getSize()){
		...
		if (rec.load_number.startsWith(scopes.jobs.prohibitedLoadPrefix)){index++;continue}//STSX task #0013 do not show deleted Load Numbers
...
function jobPcmks(topForm){
	...
	pm.and
		...
		.add(pm.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
...
function jobIdfileMiscInfo(topForm){
	...
	/** @type {QBSelect<db:/stsservoy/sheets>} */
	var s = databaseManager.createSelect('db:/stsservoy/sheets');
	s.where.add(s.columns.job_id.eq(globals.makeUUID(jobData.job_id)));
	s.result.add(s.columns.sheet_id);
	...
	id.and
		...
		.add(id.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
...
function jobStatii(topForm){
	...
	sd.and
		...
		.add(sd.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)))
		.add(sd.columns.association_id.eq(globals.makeUUID(globals.secCurrentAssociationID)))
	);
...
function queryAssembly(criteria,formName,subquery){
	...
	ad.where.add(ad.columns.job_id.eq(globals.makeUUID(jobId)));
	ad.where.add(ad.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	trans.where.add(trans.columns.job_id.eq(globals.makeUUID(jobId)));
	trans.where.add(trans.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	if (0 && doSingleLabel){
		trpm.root.where.add(trpm.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
		trpm.root.result.add(trpm.columns.piecemark_id);
	}
	trpm.root.where.add(trpm.columns.delete_flag.isNull);
	trpm.root.where.add(trpm.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	...
	trIdf.root.where.add(trIdf.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	if (0 && doSingleLabel){
		trIdf.root.where.add(trIdf.columns.idfile_id.eq(globals.makeUUID(idfilID)));
	...
	trSd.root.where.add(trSd.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	if (subquery != 'summary'){
		...
		uIdfile.where.add(uIdfile.columns.job_id.eq(globals.makeUUID(jobId)));
		uIdfile.where.add(uIdfile.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
		if (0 && doSingleLabel){
		...
		uIdpm.root.where.add(uIdpm.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
		if (0 && doSingleLabel){
			uIdpm.root.where.add(uIdpm.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
		...
		if (0 && doSingleLabel){
			uIdIdfile.root.where.add(uIdIdfile.columns.idfile_id.eq(globals.makeUUID(idfilID)));
			//uIdIdfile.root.result.add(uIdIdfile.columns.idfile_id);
		}

		uIdIdfile.root.where.add(uIdIdfile.columns.tenant_uuid.eq(globals.makeUUID(tenantId)))
		/** @type {QBJoin<db:/stsservoy/transactions>} */
		...
		uIdTrans.root.where.add(uIdTrans.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	upcmk.where.add(upcmk.columns.job_id.eq(globals.makeUUID(jobId)));
	upcmk.where.add(upcmk.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	if (criteria.sheetnuma && criteria.sheetnuma.length > 0){
	...
	upcmk.root.where.add(upmm.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	if (0 && doSingleLabel){
		...
		upmm.root.where.add(upmm.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));upcmk.groupBy.add(upmm.columns.piecemark_id);upcmk.groupBy.add(upcmk.columns.sheet_id)
	}
	...
	st.where.add(st.columns.job_id.eq(globals.makeUUID(jobId)));
	st.where.add(st.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	if (criteria.sheetnuma && criteria.sheetnuma.length > 0){
	...
	if (subquery == 'browse' && !(!jobShipToId)){
		...
		jcustAddr.root.where.add(jcustAddr.columns.address_id.eq(globals.makeUUID(jobShipToId)));
	...
	if (0 && doSingleLabel){//HERE
		...
		pmm.root.where.add(pmm.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
	}
	...
	if (doSingleLabel){
		st.where.add(pmm.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
	}
	...
	if (0 && doSingleLabel){
		...
		id1.root.where.add(id1.columns.idfile_id.eq(globals.makeUUID(idfilID)));
	}
	...
	if (subquery == 'browse'){//-------------------------------------------------------------------------------
		...
		sh3.where.add(sh3.columns.job_id.eq(globals.makeUUID(jobId)));
		sh3.where.add(sh3.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	st.where.add(sq1.columns.job_id.eq(globals.makeUUID(jobId)));
	st.where.add(sq1.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	if (subquery == "browse"){
		...
		sbs.where.add(sbs.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
		sbs.where.add(sbs.columns.delete_flag.isNull);
		sbs.where.add(sbs.columns.job_id.eq(globals.makeUUID(jobId)));
		if (criteria.sheetnuma && criteria.sheetnuma.length > 0){
		...
		if (0 && doSingleLabel){
			sbp.root.where.add(sbp.columns.piecemark_id.eq(globals.makeUUID(pcmkID)))
		...
		if (doSingleLabel){
			sbi.root.where.add(sbi.columns.idfile_id.eq(globals.makeUUID(idfilID)));
		...
		if (0 && doSingleLabel){
			id3.root.where.add(id3.columns.idfile_id.eq(globals.makeUUID(idfilID)));
...
function createRouteSummaryForm(query,formName){
	...
	stat.where.add(stat.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function viewBTableCreateForm2(formName,datasource){
	...
	var resizeMethod = checkForm.newMethod(codeResize);
	var changeSelection = checkForm.newMethod('function onDataChange(oldValue, newValue, event) {scopes.jobs.dataChangePrintSelection(oldValue, newValue, event)}');

	if (formName.search('barcode') != -1 || formName.search('print') != -1){
		code2 = 'function onRender3(event){scopes.printer.onRenderPrint(event);}';
		var code5 = 'function onRecSelectMulti(event){scopes.jobs.selectedTableIndexes = foundset.getSelectedIndexes();}';//scopes.jobs.updatePrintInventory(event);
		checkForm.onRender = checkForm.newMethod(code2);
	...
	for (var index = 0;index < columnNames4.length;index++){
		...
		if (columnName == "selection"){
			last.editable = true;
			last.horizontalAlignment = SM_ALIGNMENT.LEFT;
			last.onDataChange = changeSelection;
		}
	 	offset += colLength*1;	
	 	//if (columnName == "selection"){
...
function onDataChangeAssociation(oldValue, newValue, event) {
	...
	if (event.getFormName() == 'division_user_detail'){
		...
		assoc.where.add(assoc.columns.association_uuid.eq(globals.makeUUID(newValue)));
...
function createEmpAssocList(event){
	...
	if (!globals.session.corpUser){
		ff.root.where.add(ff.columns.association_uuid.eq(globals.makeUUID(globals.session.associationId)));
	}
	...
	ee.where.add(ee.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	us.where.add(us.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function resetSampleData(jobNumber){
	...
	id4.where.add(id4.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function createIdfilesPerf(){
	...
	for (var idx = 0;idx < pcmks.length;idx++){
		...
		rec.sequence_id = createSequenceNumber(formData.insSeqNumber);//dsSequenceArray['_'+formData.insSeqNumber];
		if (!dsIdfileListByPm[piecemarkId]){dsIdfileListByPm[piecemarkId] = []}
...
function bartenderPrintEmbed(){
	...
	t.where.add(t.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function getPiecemarkIdfiles(uniqPcmk){
	...
	r.root.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	for (barId in info.distribution){
		...
		while (ptr && ptr.length*1 < labelCapacity*1){
			...
			if (!idfileId){
				...
				tempArray[idCols.indexOf('edit_date')] = (globals.useISODate.search(/microsoft/i) != -1) ? importDate.toISOString() : "CAST(''"+importDate+"'' AS DATE)";//.toString().replace(/GMT-0500 /,'');
				...
				tempArray[idCols.indexOf('lot_id')] = createLotNumber(record.lot_number).toString();//dsLotArray["_"+record.lot_number].toString();
				tempArray[idCols.indexOf('sequence_id')] = createSequenceNumber(record.sequence_number).toString();// dsSequenceArray["_"+record.sequence_number].toString();
//				//if (record.reprint){tempArray[idCols.indexOf(searchElement)] = null}
				...
				if (record.remarks){
					if (forms['kiss_option_import'].saveNotesInto.search('Control #') != -1){
						...
						tempArray[idCols.indexOf('control_number')] = '';
					}
					if (forms['kiss_option_import'].saveNotesInto.search('Erection Dwg') != -1){
						tempArray[idCols.indexOf('erection_drawing')] = record.remarks;
					} else {
						tempArray[idCols.indexOf('erection_drawing')] = '';
					}
		...
		while (ptr && idx4 < ptr.length){
			...
			tempArray[idCols.indexOf('summed_quantity')] = summQuantity;
			tempArray[idCols.indexOf('id_serial_number_id')] = barId.toString();
			tempArray[idCols.indexOf('lot_id')] = createLotNumber(record.lot_number);//dsLotArray["_"+record.lot_number+'_'+importJob.jobId.toString()].toString();
			tempArray[idCols.indexOf('sequence_id')] = createSequenceNumber(record.sequence_number).toString();//dsSequenceArray["_"+record.sequence_number+'_'+importJob.jobId.toString()].toString();
			//if (record.reprint){tempArray[idCols.indexOf(searchElement)] = null}
			//tempArray[idCols.indexOf('original_quantity')] = seqQty;
			tempArray[idCols.indexOf('shop_order')] = forms['kiss_option_import'].jobShopOrder;
			tempArray[idCols.indexOf('id_location')] = forms['kiss_option_import'].importArea;
	...
	if (tempGlobArray.length > 0){
		...
		if (globals.databaseName().search(/microsoft/i) == -1){//mssql sql server 20210126
			var sqlUpdate = '\nUPDATE idfiles AS i SET \n'+colEquivs+
				'FROM (VALUES\n'+idArgsSql+
				'\n) AS i2'+idColsSql+
				'\nWHERE i2.idfile_id = i.idfile_id';
		} else {
			sqlUpdate = '';
			for (var tempArrIndex = 0;tempArrIndex < tempGlobArray.length;tempArrIndex++){
				sqlUpdate += 'UPDATE idfiles SET \n';
				for (var indexCols = 0; indexCols < idCols.length;indexCols++){
					if (idCols[indexCols] == 'idfile'){continue}
					sqlUpdate += ' '+idCols[indexCols]+"='"+tempGlobArray[tempArrIndex][indexCols]+"'";
					if (indexCols != idCols.length-1){sqlUpdate += ',\n'}else{sqlUpdate += '\n'}
				}
				sqlUpdate += "WHERE idfile_id = '"+tempGlobArray[tempArrIndex][idCols.indexOf('idfile_id')]+"'";
				sqlUpdate = sqlUpdate.replace(/undefined/g,'');
			}
		}
	...
	barcodeArray.sort();
	var lotId = createLotNumber(record.lot_number);//dsLotArray['_'+record.lot_number+'_'+importJob.jobId.toString()];
	var seqId = createSequenceNumber(record.sequence_number);//dsSequenceArray['_'+record.sequence_number+'_'+importJob.jobId.toString()];
...
function getPiecemarkIdfiles2(pcmkID,importRec,checkGuids){//NOPE, need sequence to identify further the idfiles for this SEQ. remove sequence ID since it is looking at unique piecemark id
	...
	if (checkGuids && importJob.guidsFabSuite){//list of guids for this seq and lot
		...
		s.where.add(s.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
	...
	var seqId = createSequenceNumber(importRec.sequence_number);//)scopes.jobs.dsSequenceArray["_"+importRec.sequence_number+'_'+importJob.jobId.toString()];
	var lotId = createLotNumber(importRec.lot_number);//scopes.jobs.dsLotArray['_'+importRec.lot_number+'_'+importJob.jobId.toString()];
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var q = databaseManager.createSelect('db:/stsservoy/idfiles');
	q.result.add(q.columns.idfile_id);
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
	q.where.add(q.columns.sequence_id.eq(globals.makeUUID(seqId)));
	q.where.add(q.columns.lot_id.eq(globals.makeUUID(lotId)));		
	var fsIdfiles = databaseManager.getFoundSet(q);//keep a foundset of the idfiles for this piecemark
...
function getIdfileBcIDs(fsIdfiles, inRec){
	...
	var rec = null;var idx = 1;
	var seqId = createSequenceNumber(inRec.sequence_number);//(!dsSequenceArray[inRec.sequence_number]) ?  createSequenceNumber(inRec.sequence_number) : dsSequenceArray[inRec.sequence_number];
	var lotId = createLotNumber(inRec.lot_number);//(!dsLotArray[inRec.lot_number]) ? createLotNumber(inRec.lot_number) : dsLotArray[inRec.lot_number];
...
function deleteEntireJob(jobId){
	...
	var sq = databaseManager.createSelect('db:/stsservoy/sequences2');
	sq.result.add(sq.columns.sequence_id);
	sq.where.add(sq.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	sq.where.add(sq.columns.job_id.eq(globals.makeUUID(jobId)));
	...
	l.where.add(l.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	l.where.add(l.columns.job_uuid.eq(globals.makeUUID(jobId)));
	...
	id3.where.add(id3.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	var ld = databaseManager.createSelect('db:/stsservoy/loads');
	ld.result.add(ld.columns.load_id);
	ld.where.add(ld.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	ld.where.add(ld.columns.job_id.eq(globals.makeUUID(jobId)));
	...
	id2.where.add(id2.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	sn.where.add(sn.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	sn.where.add(sn.columns.id_serial_number_id.isin(id2));
	...
	upm.where.add(upm.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	upm.where.add(upm.columns.job_uuid.eq(globals.makeUUID(jobId)));
...
function onLoadRemoteViews(event) {
	...
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.user_uuid.eq(globals.makeUUID(forms[event.getFormName()].user_uuid)));
...
function getImportTableGuids(importTableId,pcmkId,importRecord){
	...
	var importRecGuids = [];
	/** @type {String} */
	var seqId = createSequenceNumber(importRecord.sequence_number);//scopes.jobs.dsSequenceArray['_'+importRecord.sequence_number+'_'+importJob.jobId.toString()];
	/** @type {String} */
	var lotId = createLotNumber(importRecord.lot_number);//scopes.jobs.dsLotArray['_'+importRecord.lot_number+'_'+importJob.jobId.toString()];
	...
	h1.where.add(h1.columns.import_table_id.eq(globals.makeUUID(importTableId)));
	...
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var s = databaseManager.createSelect('db:/stsservoy/idfiles');
	s.where.add(s.columns.job_uuid.eq(globals.makeUUID(jobId)));
	s.where.add(s.columns.guid_minor.isin(importRecGuids));
	s.where.add(s.or
		.add(s.columns.sequence_id.not.eq(globals.makeUUID(seqId)))
		.add(s.columns.lot_id.not.eq(globals.makeUUID(lotId)))
		)
	var S = databaseManager.getFoundSet(s);
	if (S.getSize() > 0){
	...
	g.where.add(g.columns.piecemark_id.eq(globals.makeUUID(pcmkId)));
	...
	q.where.add(q.columns.import_table_id.eq(globals.makeUUID(importTableId)));
...
function checkMissingPMs(event){
	...
	t.where.add(t.columns.job_id.eq(globals.makeUUID(scopes.jobs.importJob.jobId)));
...
function receiveRawMaterialIntoInventory(event,jobNumber,barcode,quantity){
	...
	/** if (template.job_uuid){q.where.add(q.columns.job_uuid.eq(template.job_uuid.toString()))}
	if (template.model_part){q.where.add(q.columns.model_part.eq(template.model_part))}
	if (template.grade){q.where.add(q.columns.grade.eq(template.grade))}
	q.where.add(q.columns.tenant_uuid.eq(template.tenant_uuid.toString()));
	if (template.inventory_ref_number){q.where.add(q.columns.inventory_ref_number.eq(template.inventory_ref_number))}
	q.result.add(q.columns.inventory_uuid); */
	/** @type {JSRecord<db:/stsservoy/inventory>} */
	var Q = databaseManager.getFoundSet(q);
	// do something with import
	var recIdx = Q.newRecord();
	if (!Q.inventory_uuid){application.output('there is no UUID for new inventory record. Creating.');Q.inventory_uuid = application.getUUID()}
	application.output('New Inv Record Index: '+recIdx);
	/** @type {JSRecord<db:/stsservoy/inventory>} */
...
function invHeat(formName){
	...
	var q = databaseManager.createSelect('db:/stsservoy/inventory');
	q.columns.heat;
	q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid));
	q.columns.association_uuid.eq(globals.makeUUID(globals.session.associationId));
	if (jobID){
		q.columns.job_uuid.eq(globals.makeUUID(jobId));
	}
	q.result.distinct = true;
...
function verifyCutListItems(event,cutListObject){
	...
	for (var idx = 0;idx < cutListObject.cutarray.length;idx++){
		...
		application.output('RM getPiecemarkToCut '+!(!idfileRec)+ ' Main:'+item.pMajor+' Minor:'+item.pMinor);
		if (idfileRec){// && scopes.prefs.lFsPrintIDFromCutList == "1"){
			...
			r.where.add(r.columns.id_serial_number_id.eq(idfileRec.id_serial_number_id));
			...
			application.output('No idfile/piecemark for piecemark: '+item.pMajor+' '+item.pMinor+' '+item.pMaterial+' '+item.pJob);
			item['available'] = null;//20210302 no process when there is no piecemark to track
		}
		if (1 || application.isInDeveloper()){
			var itemData = '';
...
function getPiecemarkIdUnused(jobIdOrNum,major,minor,sheetNum,grade,finish,material){
	...
	sheetId = getSheetIdUnused(jobId,sheetNum);
	/** @type {QBSelect<db:/stsservoy/piecemarks>} */
	var q = databaseManager.createSelect('db:/stsservoy/piecemarks');
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.job_uuid.eq(globals.makeUUID(jobId)));
	...
	q.where.add(q.columns.sheet_id.eq(globals.makeUUID(sheetId)));
	q.where.add(q.columns.grade.eq(grade));
	q.where.add(q.columns.finish.eq(finish));
	q.result.add(q.columns.piecemark_id);
	var Q = databaseManager.getFoundSet(q);
...
function getSheetIdUnused(jobIdOrNum,sheetNum){
	...
	var q = databaseManager.createSelect('db:/stsservoy/sheets');
	q.where.add(q.columns.job_id.eq(globals.makeUUID(jobId)));
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.makeUUID(globals.session.tenant_uuid))));
...
function getPiecemarkToCut(jobIdOrNum,major,minor,sheetNum,grade,finish,material,lotNum,seqNum,strikeThru){
	...
	var p = databaseManager.createSelect('db:/stsservoy/piecemarks');
	p.where.add(p.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	p.where.add(p.columns.job_uuid.eq(globals.makeUUID(jobId)));
	...
	if (P.getMaxRowIndex() == 0){//in the case of grade substitutions 20210226 as per pp Stinger
		p.where.clear();
		p.result.clear();
		...
		p.where.add(p.columns.piecemark.eq(minor));
		p.result.add(p.columns.piecemark_id);
		P = databaseManager.getDataSetByQuery(p,-1);
	}
	var pIdArray = P.getColumnAsArray(1);var avail = [null,''];
	//for (var jjj = 0;jjj < pIdArray.length;jjj++){
	//	pIdArray[jjj] = globals.makeUUID(pIdArray[jjj]);
	//}
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var q = databaseManager.createSelect('db:/stsservoy/idfiles');
	...
	//q.where.add(q.columns.lot_id.eq(globals.makeUUID(lotId)));
	//q.where.add(q.columns.sequence_id.eq(globals.makeUUID(seqId)));
	q.where.add(q.columns.raw_material_id.isin(avail));
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	//if (lotId != "-1"){
	//}
	//if (seqId != "-1"){
...
function getLotId(jobIdOrNum,lotNum){
	...
	var Q = databaseManager.getDataSetByQuery(q,-1);
	if (Q.getMaxRowIndex() > 0){
		//if (sendArray){return Q.getColumnAsArray(1).sort())}
		Q.rowIndex = 1;
		return Q.lot_id;
	}
	return "-1";
...
function getSeqId(jobIdOrNum,seqNum){
	...
	var Q = databaseManager.getDataSetByQuery(q,-1);
	if (Q.getMaxRowIndex() > 0){
		//if (sendArray){return Q.getColumnAsArray(1)}
		Q.rowIndex = 1;
...
function scanCutListPiecemarkBarcodes(event,barcode){
	...
	r.where.add(r.columns.id_serial_number_id.eq(globals.makeUUID(id_serial_id)));
	r.where.add(r.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	t.where.add(t.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	t.where.add(t.columns.sequence_id.eq(globals.makeUUID(seqID)));
...
function createViewTable(dataSet){
	...
	while (rec = formFS.getRecord(idx++)){
		...
		viewRec.where.add(viewRec.columns.piecemark_uuid.eq(globals.makeUUID(pcmkID_UUID)));
		viewRec.where.add(viewRec.columns.idfile_uuid.eq(globals.makeUUID(idfileID_UUID)));
		viewRec.where.add(viewRec.columns.id_serial_uuid.eq(globals.makeUUID(barcodeID_UUID)));
...
function queryAssemblyFaster(criteria,formName,subquery){
	...
	if (subquery != 'singleLabel'){
		...
		q.where.add(q.columns.job_id.eq(globals.makeUUID(jobID)));
		q.where.add(q.columns.piecemark_id.eq(globals.makeUUID(pcmkID)));
		q.where.add(q.columns.idfile_id.eq(globals.makeUUID(idfilID)));
	...
	if (doSingleLabel){
		q.where.add(q.columns.piecemark_uuid.eq(globals.makeUUID(pcmkID)));
		q.where.add(id.columns.idfile_id.eq(globals.makeUUID(idfilID)));
	}
	...
	if (subquery != 'stations'){
		...
		q.groupBy.add(id.columns.summed_quantity);//added jmc 20201211 mssql
	}
	if (1==1){return q}
	//var Q = databaseManager.getFoundSet(q);
...
function getUniqPcmkEntry(job_uuid, idfile_uuid, piecemark_uuid, sheet_uuid,lot_uuid,id_serial_uuid, sequence_uuid,pcmkCount,tenant_uuid,pcmkTotal,bcCount){
	/** @type {QBSelect<db:/stsservoy/pcmk_instances>} */
	var q = databaseManager.createSelect('db:/stsservoy/pcmk_instances');
	q.where.add(q.columns.id_serial_uuid.eq(globals.makeUUID(id_serial_uuid)));
	q.where.add(q.columns.piecemark_uuid.eq(globals.makeUUID(piecemark_uuid)));
	...
	if (Q.getSize() == 0){
		...
		/** @type {JSFoundSet<db:/stsservoy/pcmk_instances>} */		
		var rec = Q.getRecord(idx);
		rec.job_uuid = globals.makeUUID(job_uuid);
		rec.piecemark_uuid = globals.makeUUID(piecemark_uuid);
		rec.id_serial_uuid = globals.makeUUID(id_serial_uuid);
		rec.tenant_uuid = globals.makeUUID(globals.session.tenant_uuid);
	} else {
		rec = Q.getRecord(1);
		rec.edit_date = new Date();
	}
	rec.sequence_uuid = globals.makeUUID(sequence_uuid);
	rec.sheet_uuid = globals.makeUUID(sheet_uuid);
	rec.lot_uuid = globals.makeUUID(lot_uuid);
	rec.pcmk_total = pcmkTotal;
	rec.pcmk_bc_count = bcCount;
	rec.pcmk_count = pcmkCount;
	rec.idfile_uuid = globals.makeUUID(idfile_uuid);
	databaseManager.saveData(rec);
	if (!importJob.updatePcmkInstances){importJob.updatePcmkInstances = []}
...
function sqlArgsToSqlData(sqlArgsArr){
	...
	if (globals.useISODate.search(/microsoft/i) != -1){
	...
	if (0 && application.isInDeveloper()){application.output('sqlArgs replace date:'+sqlArgs)}
	return sqlArgs;
...
function resequenceIdfilesImport(record){
	...
	var p = databaseManager.createSelect('db:/stsservoy/piecemarks');
	p.where.add(p.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	p.where.add(p.columns.job_uuid.eq(record.job_id));
	...
	if (P.getMaxRowIndex() > 0){
		var lotId = createLotNumber(lotNum);//(!dsLotArray['_'+lotNum]) ? createLotNumber(lotNum) : dsLotArray['_'+lotNum];
		var seqId = createSequenceNumber(seqNum);//(!dsSequenceArray['_'+seqNum]) ? createSequenceNumber(seqNum) : dsSequenceArray['_'+seqNum];
		...
		m.where.add(m.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
		m.where.add(m.columns.job_uuid.eq(globals.makeUUID(jobId)));
...
function getCurrentPcmkJoins(record,piecemarkId){
	...
	var q = databaseManager.createSelect('db:/stsservoy/pcmk_instances');
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.job_uuid.eq(globals.makeUUID(importJob.jobId)));
	q.where.add(q.columns.piecemark_uuid.eq(globals.makeUUID(piecemarkId)));
	q.where.add(q.columns.sequence_uuid.eq(globals.makeUUID(createSequenceNumber(record.sequence_number))));//dsSequenceArray['_'+record.sequence_number])));
	q.where.add(q.columns.lot_uuid.eq(globals.makeUUID(createLotNumber(record.lot_number))));//dsLotArray['_'+record.lot_number])));
...
function deleteNonImportGuids(){
	...
	r.where.add(r.columns.job_id.eq(globals.makeUUID(importJob.jobId)));
	r.where.add(r.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	...
	u.where.add(u.columns.job_id.eq(globals.makeUUID(importJob.jobId)));
	u.where.add(u.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	/** @type {QBJoins<db:/stsservoy/import_guids>} */
	var v = u.joins.add('db:/stsservoy/import_guids');
	v.on.add(u.columns.import_table_id.eq(v.columns.import_table_id));
	v.root.where.add(t.columns.guid_minor.not.isin(v.columns.part_guid))
	//u.result.add(v.columns.part_guid);
	//u.sort.add(v.columns.part_guid.asc);//RMM mssql
	//u.groupBy.add(v.columns.part_guid);//RMM jmc 20201211 mssql
	//var V = databaseManager.getDataSetByQuery(r,-1);
	
	/** @type {QBSelect<db:/stsservoy/import_guids>} */
	var xx = databaseManager.createSelect('db:/stsservoy/import_guids');
	xx.where.add(xx.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	xx.where.add(xx.columns.job_id.eq(globals.makeUUID(importJob.jobId)));
	xx.result.add(xx.columns.part_guid);
	
	//r.where.add(r.exists(t.columns.guid_minor.not.isin(v.columns.part_guid).root))//subquery example???
	r.where.add(r.not(t.columns.guid_minor.isin(xx)));
	r.result.add(t.columns.guid_major);
	r.result.add(t.columns.guid_minor);
	r.result.add(t.columns.idfile_id);
	r.sort.add(t.columns.idfile_id.asc);//RMM jmc mssql 20201211 removed for
	...
	x.where.add(x.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	x.where.add(x.columns.job_uuid.eq(globals.makeUUID(importJob.jobId)));
...
function cutoverEmployeeDBF(event){
	...
	r.where.add(r.columns.tenant_uuid.eq(globals.makeUUID(tenantId)));
	...
	for (idx = 1;idx <= maxRows;idx++){
		...
		newRec.logic_flag = 2;//true;//(Q.logicflag);
...
function cutoverEmployeeClassDBF(){
	...
	for (idx = 1;idx <= maxRows;idx++){
		...
		//newRec.association_uuid = assocID;
		newRec.tenant_uuid = tenantId;
		newRec.logic_flag = 2;//(Q.logicflag);
		newRec.edit_date = (Q.editdate) ? globals.dateAndTimeToDate(Q.editdate,Q.edittime) : impDate;
...
function cutoverStatusDescriptionDBF(event){
	...
	lc.where.add(lc.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
...
function cutoverLoadsDBF(event){
	...
	var jobMap = [];
	var carrierMap = [];
	var table = databaseManager.getTable('db:/dbf_data/load');
	var tableCols = table.getColumnNames();
	/** @type {QBSelect<db:/dbf_data/loadx>} */
	var q = databaseManager.createSelect('db:/dbf_data/load');
	for (var idx = 0;idx < tableCols.length;idx++){
		q.result.add(q.columns[tableCols[idx]]);
	}
	...
	for (index = 1;index <= Q.getMaxRowIndex();index++){
		...
		if (jobUUID){
			r.where.add(r.columns.job_id.eq(globals.makeUUID(jobUUID)));
		}
		...
		if (Q.jobid){
			if (jobIdMap[Q.jobid]){
				if (jobMap[jobIdMap[Q.jobid]]){
					...
					application.output('Q.jobid '+Q.jobid);
					var jobInfo = globals.getJobIdInfo(jobIdMap[Q.jobid])
					jobMap[jobIdMap[Q.jobid]] = jobInfo.job_id;
					jobUUID = jobInfo.job_id;
				}
...
function cutoverPiecemarksDBF(event){
	...
	for (index = 1;index <= Q.getMaxRowIndex();index++){
		...
		r.where.add(r.columns.sheet_id.eq(globals.makeUUID(sheetUUID)));
		r.where.add(r.columns.job_uuid.eq(globals.makeUUID(jobUUID)));
 ...
 *
 * @properties={typeid:24,uuid:"35736A5E-7168-4C00-AA2A-5B57DC89AAC2"}
 */
function getCutListDuration(cutData,cutAll){
	var form = forms['rf_mobile_view'];
	/** @type {Array} */
	var cuts = cutData.cutarray;
	var cutsCount = 0;
	...
	var endDur = endTimed.getTime();
	var begDur = new Date(globals.mob.timedBegin).getTime();
	var duration = Math.floor((endDur - begDur)/cutCount);
	var durationMins = duration/1000/60;
	var durs = durationMins.toString().split('.');
	durationMins  = durs[0];
	if (durs.length == 2){durationMins = durationMins +'.'+(durs[1].substr(0,3)+.005).substr(0,2);}
	var bDate = new Date(globals.mob.timedBegin).getTime();
	var endTime = new Date(bDate + duration);
	globals.session.cutDuration = {
		startCut : globals.mob.timedBegin,
		endCut : endTime,
 ...
 *
 * @properties={typeid:24,uuid:"2A3E3A8F-9770-4F9C-BF0F-689BE498F180"}
 */
function updatePrintInventory(event,selected){
	application.output('update STSX Inventory from EPM');
	var formName = event.getFormName();
	var form = forms[formName];
	var fs = form.foundset;
	if (!selected){
		selected = fs.getSelectedIndexes();
	}
	if (selected.length > 0){
		for (var index = 0;index < selected.length;index++){
			/** @type {JSRecord<db:/stsservoy/inventory>} */
			var rec = fs.getRecord(selected[index]);
			if (rec.selection*1 == 1){continue}
			var serialNumber = rec.serial_number;
			var changeRec = scopes.jobs.invUpdate(event,serialNumber);
			if (changeRec){
				var changed = databaseManager.copyMatchingFields(changeRec,rec,true);
			}
			if (application.isInDeveloper()){application.output(changeRec)}
			if (application.isInDeveloper()){application.output(rec);}
		}
	}
}
 ...
 * @param {String} serialNumber
 *
 * @properties={typeid:24,uuid:"B4A0DD8A-CF11-41A2-9EED-90949CFFED6B"}
 */
function invUpdate(event,serialNumber){
	if (!globals.fabsuiteConnected){
		if (scopes.fs.checkComFabsuite(event)){
			globals.fabsuiteConnected = true;			
		} else {
			globals.fabsuiteConnected = false;
		}
		
	}
	/** @type {{locationsts:String,po_number:String,bill_of_lading_in:String,job_number:String,coo:String,item_length:Number,item_length_in:Number,item_length_char:String,item_weight:Number,item_weight_lbs:Number,item_width:Number,item_width_in:Number,grade:String,heat:String,model_part:String}} */
	var updateInfo = scopes.fs.getInventorySerial(event,serialNumber);
	//for (var i in updateInfo){
	//	application.output(i+': '+updateInfo[i])
	//}
	null;
	/** @type {QBSelect<db:/stsservoy/inventory>} */
	var q = databaseManager.createSelect('db:/stsservoy/inventory');
	...
	if (Q.getSize() == 1){
		/** @type {JSFoundSet<db:/stsservoy/inventory>} */
		var rec = Q.getRecord(1);
		if (updateInfo.job_number){
			/** @type {{job_id:String}} */
			var jobInfo = globals.getJobIdInfo(updateInfo.job_number);
			if (rec.job_uuid != jobInfo.job_id){rec.job_uuid = jobInfo.job_id;}
		}
		if (rec.item_length != updateInfo.item_length){rec.item_length = updateInfo.item_length;}
		if (rec.item_length_in != updateInfo.item_length_in){rec.item_length_in = updateInfo.item_length_in;}
		if (rec.item_length_char != updateInfo.item_length_char){rec.item_length_char = updateInfo.item_length_char;}
		if (rec.bill_of_lading_in != updateInfo.bill_of_lading_in){rec.bill_of_lading_in = updateInfo.bill_of_lading_in;}
		if (rec.item_weight != updateInfo.item_weight){rec.item_weight = updateInfo.item_weight;}
		if (rec.item_weight_lbs != updateInfo.item_weight_lbs){rec.item_weight_lbs = updateInfo.item_weight_lbs;}
		if (rec.item_width != updateInfo.item_width){rec.item_width = updateInfo.item_width;}
		if (rec.item_width_in != updateInfo.item_width_in){rec.item_width_in = updateInfo.item_width_in;}
		if (rec.po_number != updateInfo.po_number){rec.po_number = updateInfo.po_number;}
		if (rec.location != updateInfo.locationsts){rec.location = updateInfo.locationsts;}
		if (rec.heat != updateInfo.heat){
			rec.heat = updateInfo.heat;
			rec.heat_uuid = globals.createHeat(event,updateInfo.heat);
		}
		if (rec.grade != updateInfo.grade){rec.grade = updateInfo.grade;}
		if (rec.model_part != updateInfo.model_part){rec.model_part = updateInfo.model_part;}
		if (rec.country_of_origin != updateInfo.coo){rec.country_of_origin = updateInfo.coo;}
		if (rec.hasChangedData()){
			databaseManager.saveData(rec);
 ...
 * @param {JSEvent} event
 *
 * @properties={typeid:24,uuid:"FD2E98ED-FFBE-4CE5-9D2E-4B23137F754D"}
 */
function dataChangePrintSelection(oldValue, newValue, event){
	var selectedIndexes
	if (newValue*1 == 1){
		//updatePrintInventory(event);
	}
}
/**
 * @type {JSEvent} event
...
function cutoverIdfilesDBF(event){
	 ...
	 * for that job,  look up the text sheetnum
	 * for that sheetnum, look up the piecemark
	 * has barcode 'id' which must be created in serial numbers
	 * needed:
	 * job_uuid, piecemark_id, sequence_id, lot_id, original_quantity, tenant_uuid, id_serial_number_id, status_description_id, guid_major, guid_minor, 
	 * original_employee_uuid
	 * 
	 */
	var impDate = new Date();
	...
	m.result.add(m.columns.jobid);
	m.result.add(m.columns.jobnum);
	/** @type {JSFoundSet<db:/dbf_data/jobs>} */
	var M = databaseManager.getDataSetByQuery(m,-1);
	for (index = 1;index <= M.getMaxRowIndex();index++){
		M.rowIndex = index;
		if (jobIdMap[M.jobid]){application.output(jobIdMap[M.jobid]+' M.dupe job id to number '+M.jobid+' '+M.jobnum);continue}
		jobIdMap[M.jobid] = M.jobnum;
		//application.output('jobid: '+M.jobid+'job: '+M.jobnum);
	}	
	var jobMap = [];
	...
	var tableCols = table.getColumnNames();
	/** @type {QBSelect<db:/dbf_data/idfile>} */
	var q = databaseManager.createSelect('db:/dbf_data/idfile');
	for (var idx = 0;idx < tableCols.length;idx++){
		q.result.add(q.columns[tableCols[idx]]);
	}
	q.sort.add(q.columns.jobid.asc);
	/** @type {JSFoundSet<db:/dbf_data/idfile>} */
	var Q = databaseManager.getDataSetByQuery(q,-1);
	...
	for (index = 1;index <= Q.getMaxRowIndex();index++){
		...
		if (Q.jobid){
			if (jobIdMap[Q.jobid]){
				if (jobMap[jobIdMap[Q.jobid]]){
					...
					jobMap[jobIdMap[Q.jobid]] = jobInfo.job_id;
					jobUUID = jobInfo.job_id;
					if (jobInfo.job_rec.logic_flag*1 != 2){
						if (skipJob.indexOf(jobIdMap[Q.jobid]) == -1){
							skipJob.push(jobInfo.job_id);
						}
					}
				}
				jobMetric[jobUUID] = Q.metricjob;
		...
		if (skipJob.indexOf(jobUUID) != -1){continue}
		//var grade = (Q.grade) ? Q.grade.trim() : '';
		var finish = (Q.idfinish) ? Q.idfinish.trim() : '';
		...
		if (R.getSize() == 0){
			application.output('skipping: '+Q.pcmark+' '+Q.pcmark+' '+Q.idstatus+' '+Q.jobid+finish);
			continue;
		} else {
			newRecCount++;
			indexCount++;
			if (started){
				if (!sheetUUID){
					application.output(Q.jobid+' '+Q.pcmark+' '+' '+Q.sheetnum+' '+sheetUUID+' ');
				}
				/** @type {QBSelect<db:/stsservoy/idfiles>} */
				var s = databaseManager.createSelect('db:/stsservoy/idfiles');
				var S = databaseManager.getFoundSet(s);
				/** @type {JSFoundSet<db:/stsservoy/idfiles>} */
				var rec = S.getRecord(S.newRecord());
				rec.logic_flag = 2;//Q.logicflag;
				rec.tenant_uuid = globals.session.tenant_uuid;
				rec.edit_date = (Q.editdate) ? globals.dateAndTimeToDate(Q.editdate,Q.edittime) : null;
				if (Q.delete_dt){rec.delete_flag = true;}
				rec.job_uuid = globals.makeUUID(jobUUID);
				rec.piecemark_id = globals.makeUUID(R.getRecord(1).piecemark_id);
				...
				rec.guid_minor = Q.guid;//minorguid
				rec.off_hold_date = (Q.offholdate) ? globals.dateAndTimeToDate(Q.offholdate,null) : null;//offholddate
				rec.cost_date = (Q.costdate) ? globals.dateAndTimeToDate(Q.costdate,null) : null;;//costdate
				rec.ut_test = Q.uttst;//uttst
				rec.sds_member = Q.sds_member;//sds_member
				
				rec.shipped_quantity = (Q.shipqty) ? Q.shipqty : null;//shipqty
				rec.receive_quantity = (Q.recvqty) ? Q.recvqty : null;//recvqty
				rec.object_id = (Q.objectid) ? Q.objectid.toString().trim() : '';//objectid
				rec.longitude = (Q.longitude) ? Q.longitude.toString().trim() : '';//longitude
				rec.latitude = (Q.latitude) ? Q.latitude.toString().trim() : '';//latitude
				rec.altitude = (Q.altitude) ? Q.altitude.toString().trim() : '';//altitude
				...
				rec.on_hold_date = (Q.onholdate) ? globals.dateAndTimeToDate(Q.onholdate,null) : null;//onholddate
				rec.lprint = (Q.lprint) ? true : null;//lprint
				rec.pmi_test = (Q.pmitst) ? Q.pmitst : null;//pmitest
				rec.stress_test = (Q.stresstst) ? Q.stresstst : null;//stresstst
				rec.bhn_test = (Q.bhntst) ? Q.bhntst : null;//bhntest
				...
				rec.bundle_bc = (Q.bundleid) ? Q.bundleid.trim() : null;//bundleid
				rec.id_on_hold = Q.idonhold;//idonhold
				rec.print_date = (Q.print_dt) ? globals.dateAndTimeToDate(Q.print_dt,null) : null;//print_dt
				//UNUSED recaction
				//recList.push(rec);
				// origqty = qty on this serial number, labelqty = number of barcodes used this pcmk, labelbc=total pcs on this barcode
				var guidArray = [];
				if (itemCount > 1){
					// collect guid for main mark in an array, then pop into each copy of the idfile
					/** @type {QBSelect<db:/dbf_data/id_file_multiguid>} */
					var b = databaseManager.createSelect('db:/dbf_data/id_file_multiguid');
					b.where.add(b.columns.jobid.eq(Q.jobid));
					b.where.add(b.columns.parentid.eq(Q.id));
					b.result.add(b.columns.multiguid);
					var B = databaseManager.getDataSetByQuery(b,-1);
					guidArray = B.getColumnAsArray(1);
					rec.guid_minor = guidArray.pop();
				}
				for (var itemCount = Q.labelbc;itemCount > 1;itemCount--){
					/** @type {JSFoundSet<db:/stsservoy/idfiles>} */
					var addlRec = S.getRecord(S.newRecord());
					databaseManager.copyMatchingFields(rec,addlRec,true);
					addlRec.guid_minor = guidArray.pop();
					newRecCount++;
				}
		...
		application.output('index; '+indexCount+' newreccount: '+newRecCount);
		application.output('idfiles record major: '+Q.pcmark+' '+Q.pcmark);
		if (newRecCount > 50){
			newRecCount = 0;
			var newRec = null;
			////while (newRec = recList.pop()){
			...
			//}
			databaseManager.commitTransaction();
			var saveResponse = databaseManager.saveData(S);
	...
	for (index = 1;index <= M.getMaxRowIndex();index++){
		...
		if (!matched || Q.getSize() > 0){
			if (!matched){
				...
				var msg = i18n.getI18NMessage('sts.txt.crossover.last.id.exists')+' '+rec.prefix+' incoming: '+dbfPrefix;
				warningsMessage(msg,true);
			}
 ...
 *
 *
 * @properties={typeid:24,uuid:"FF76952B-A55D-446D-9697-28B2D27B34E8"}
 */
function getSeqIdArray(jobIdOrNum,seqNum){
	var jobId = jobIdOrNum;
	var jobNum;
	if (jobIdOrNum && jobIdOrNum.toString().length == 36){
		jobId = jobIdOrNum;
 ...
 *
 *
 * @properties={typeid:24,uuid:"0DD116D7-02E6-4696-BDEA-93E3F471DBB1"}
 */
function getLotIdArray(jobIdOrNum,lotNum){
	var jobId = jobIdOrNum;
	var jobNum;
	if (jobIdOrNum.toString().length == 36){
		jobId = jobIdOrNum;
	...
	var Q = databaseManager.getDataSetByQuery(q,-1);
	if (Q.getMaxRowIndex() > 0){
		return Q.getColumnAsArray(1).sort();
	}
	return [];
}
/**
 ...
 * @param jobNum
 *
 * @properties={typeid:24,uuid:"8B11E297-1640-4A33-901E-5A6AC33CF1FA"}
 */
function buildPcmkInstances(event, jobNum){
	var tenantId = globals.makeUUID(globals.session.tenant_uuid);
	var jobInfo = globals.getJobIdInfo(jobNum);
	var jobId = globals.makeUUID(jobInfo.job_id);
	/** @type {QBSelect<db:/stsservoy/pcmk_instances>} */
	var q = databaseManager.createSelect('db:/stsservoy/pcmk_instances');
	q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
	q.where.add(q.columns.job_uuid.eq(globals.makeUUID(jobId)));
	//var Q = databaseManager.getFoundSet(q);
	/**
	 * get piecemark and collect idfiles for that piecemark
	 * get total piecemark count in idfiles
	 * get unique serials for idfiles
	 * idfiles by seqId, lotId, serialId
	 * job_uuid, idfile_uuid, piecemark_uuid, sheet_uuid, lot_uuid, sequence_uuid, id_serial_uuid, tenant_uuid,
	 * ADD pcmk_count, pcmk_total, edit_date, pcmk_bc_count
	 * 
	 * uses a representative idfile, not all idfile ids
	 * 
	 */
	var rec = null;var idx = 1;var text = '';var idxC = 0;var killRec = [];
	//while (rec = Q.getRecord(idx++)){
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var r = databaseManager.createSelect('db:/stsservoy/idfiles');
	r.result.distinct = true;
	r.where.add(r.columns.job_uuid.eq(jobId));
	r.where.add(r.columns.tenant_uuid.eq(tenantId));
	r.groupBy.add(r.columns.id_serial_number_id);
	r.groupBy.add(r.columns.piecemark_id);
	r.groupBy.add(r.columns.sequence_id);
	r.groupBy.add(r.columns.lot_id);
	//r.groupBy.add(r.columns.guid_minor);
	//r.groupBy.add(r.columns.idfile_id);
	r.result.add(r.columns.id_serial_number_id);
	r.result.add(r.columns.piecemark_id);
	r.result.add(r.columns.sequence_id);
	r.result.add(r.columns.lot_id);
	//r.result.add(r.columns.guid_minor);
	//r.result.add(r.columns.idfile_id);

	r.result.add(r.columns.summed_quantity.sum,'pcmk_count');
	r.sort.add(r.columns.id_serial_number_id.asc);
	var P = databaseManager.getDataSetByQuery(r,-1)
	
	idx = 1;var guidMinorSeen = [];
	var Q = null;var noMatches = 0; var hasMatches = 0;var createShorts = [];
	for (idx = 1;idx <= P.getMaxRowIndex();idx++){
		P.rowIndex = idx;
		//var guidMinor = P.guid_minor;
		q.where.clear();
		q.where.add(q.columns.tenant_uuid.eq(globals.makeUUID(globals.session.tenant_uuid)));
		q.where.add(q.columns.job_uuid.eq(globals.makeUUID(jobId)));
		q.where.add(q.columns.piecemark_uuid.eq(P.piecemark_id));
		q.where.add(q.columns.id_serial_uuid.eq(P.id_serial_number_id));
		q.where.add(q.columns.idfile_uuid.eq(P.idfile_id));
		var Q = databaseManager.getFoundSet(q);
		if (Q.getSize() > 1){application.output('Too many unique idfiles represented')}
		if (Q.getSize() == 1){hasMatches++}
		if (Q.getSize() == 0){
			var newItem = P.piecemark_id+ '|'+P.sequence_id+'|'+P.lot_id+'|'+P.id_serial_number_id+'|'+P.pcmk_count;
			if (createShorts.indexOf(newItem) == -1){
				createShorts.push(newItem);
				noMatches++;
			}
		}
		
		
		/** var unique = P.piecemark_id+'|'+P.sequence_id+'|'+P.lot_id+'|'+P.id_serial_number_id;
		var targetId = P.idfile_id+'|'+P.guid_minor;
		if (!guidMinorSeen[guidMinor]){guidMinorSeen[guidMinor] = []}
		if (!guidMinorSeen[guidMinor][unique]){guidMinorSeen[guidMinor][unique] = []}
		if (guidMinorSeen[guidMinor][unique].indexOf(targetId) == -1){guidMinorSeen[guidMinor][unique].push(targetId)}
		*/
		//if (P.pcmk_count == 1){continue}
		//text += P.piecemark_id+ ' | '+P.sequence_id+' | '+P.lot_id+' | '+P.guid_minor+' | '+P.id_serial_number_id+' | '+P.pcmk_count+'\n';
		///text += P.piecemark_id+ ' | '+P.sequence_id+' | '+P.lot_id+' | '+P.guid_minor+' | '+P.id_serial_number_id+' | '+P.pcmk_count+'\n';
	}
	null;
	text += '\nCompleted\nMatches:'+hasMatches+' No Matches: '+noMatches;
	application.output(text);var text2 = '';
	createPcmkInstances(event,jobId,createShorts)
	/** for (var minor in guidMinorSeen){
		if (1=1){continue}
		text2 += '\n'+guidMinorSeen[minor]+':';
		for (var targId in guidMinorSeen[minor]){
			text2 += guidMinorSeen[minor][targId];
		}
	}
	application.output(text2);
	*/
	if (1) {return}
	if (Q.getSize() == 0){
		var idx = Q.newRecord();
	...
	/** @type {QBSelect<db:/stsservoy/piecemarks>} */
	var p = databaseManager.createSelect('db:/stsservoy/piecemarks');
	p.where.add(p.columns.tenant_uuid.eq(tenantId));
	p.where.add(p.columns.job_uuid.eq(jobId));
	p.result.add(p.columns.piecemark_id);
 ...
 *
 * @properties={typeid:24,uuid:"BA597E60-4B0F-4B3D-AD50-20EFA4CFFF39"}
 */
function createPcmkInstances(event,jobId,createArray){
	//P.piecemark_id+ '|'+P.sequence_id+'|'+P.lot_id+'|'+P.id_serial_number_id+'|'+P.pcmk_count;
	var tenantId = globals.makeUUID(globals.session.tenant_uuid);
 	jobId = globals.makeUUID(jobId);
	/** @type {QBSelect<db:/stsservoy/pcmk_instances>} */
	var q = databaseManager.createSelect('db:/stsservoy/pcmk_instances');
	...
	var Q = databaseManager.getFoundSet(q);
	/** @type {QBSelect<db:/stsservoy/idfiles>} */
	var l = databaseManager.createSelect('db:/stsservoy/idfiles');
	l.where.add(l.columns.tenant_uuid.eq(tenantId));
	l.where.add(l.columns.job_uuid.eq(jobId));
	/** @type {QBSelect<db:/stsservoy/piecemarks>} */
	var m = databaseManager.createSelect('db:/stsservoy/piecemarks');
	/** @type {JSRecord<db:/stsservoy/pcmk_instances>} */
	var rec = null;var count = 0;
	var editDate = new Date();
	databaseManager.startTransaction();
	var item = '';
	while (item = createArray.pop()){
		var items = item.split('|');
		var pId = items[0];
		var sId = items[1];
		var lId = items[2];
		var iId = items[3];
		var pcmk_count = items[4];
		l.where.clear();
		l.result.clear();
		l.where.add(l.columns.tenant_uuid.eq(tenantId));
		l.where.add(l.columns.job_uuid.eq(jobId));
		l.where.add(l.columns.id_serial_number_id.eq(iId));
		l.result.add(l.columns.idfile_id);
		l.result.add(l.columns.original_quantity);
		var L = databaseManager.getDataSetByQuery(l,-1);
		L.rowIndex = 1;
		
		m.where.clear();
		m.result.clear();
		m.where.add(m.columns.tenant_uuid.eq(tenantId));
		m.where.add(m.columns.job_uuid.eq(jobId));
		m.where.add(m.columns.piecemark_id.eq(pId));
		m.result.add(m.columns.pcmk_qty);
		var M = databaseManager.getDataSetByQuery(m,-1);
		M.rowIndex = 1;
		var pcmk_total = (M.getMaxRowIndex() == 1) ? M.pcmk_qty : 0;
		
		
		
		rec = Q.getRecord(Q.newRecord());
		count++;
		rec.piecemark_uuid = pId;
		rec.sequence_uuid = sId;
		rec.lot_uuid = lId;
		rec.tenant_uuid = tenantId;
		rec.job_uuid = jobId;
		rec.edit_date = editDate;
		rec.id_serial_uuid = iId;
		rec.idfile_uuid = L.idfile_id;
		rec.pcmk_bc_count = L.original_quantity;
		rec.pcmk_total = pcmk_total;
		rec.pcmk_count = pcmk_count;
		if (count/200 == Math.floor(count/200)){
			try {
			databaseManager.commitTransaction();
			} catch (e) {
				application.output('Rollback necessary. Bailing this record: '+items);
				databaseManager.rollbackTransaction();
				databaseManager.startTransaction();
				break;
			}
 ...
 * @param event
 *
 * @properties={typeid:24,uuid:"37AD97F9-CAC0-44B5-BE08-875886B3B190"}
 */
function cutoverPcmkInstances(event){
	var impDate = new Date();
	var startTime = new Date().getTime();
	var currJobNumId = '';
	var index = 1;
	...
	for (index = 1;index <= M.getMaxRowIndex();index++){
		...
		jobIdMap[M.jobid] = M.jobnum;
		//application.output('jobid: '+M.jobid+'job: '+M.jobnum);
		jobArrayList.push(M.jobnum);
	}
	warningsMessage('Instancing jobs',true);
	for (var idx = 0;idx < jobArrayList.length;idx++){
		var jobNumber = jobArrayList[idx];
		var jobInfo = globals.getJobIdInfo(jobNumber);
		if (jobInfo.job_rec.logic_flag != 2){continue}//getting error on import of a new job
		warningsMessage('Instancing job '+jobNumber,true);
		scopes.jobs.buildPcmkInstances(event,jobNumber);
	}
	warningsX(event);
}
/**
 ...
 * @param event
 *
 * @properties={typeid:24,uuid:"CF61D1D8-A585-4309-88F4-979E5C981AB8"}
 */
function cutoverIdfilesMinorsDBF(event){
	/**
	 * ifile.dbf
	 * jobid is equivalent to jobIdMap[jobid]
	 * for that job,  look up the text sheetnum
	...
	var table = databaseManager.getTable('db:/dbf_data/sheetbom');
	var tableCols = table.getColumnNames();
	/** @type {QBSelect<db:/dbf_data/sheetbom>} */
	var q = databaseManager.createSelect('db:/dbf_data/sheetbom');
	for (var idx = 0;idx < tableCols.length;idx++){
		q.result.add(q.columns[tableCols[idx]]);
	}
	q.sort.add(q.columns.jobid.asc);
	/** @type {JSFoundSet<db:/dbf_data/sheetbom>} */
	var Q = databaseManager.getDataSetByQuery(q,-1);
	...
	for (index = 1;index <= Q.getMaxRowIndex();index++){
		...
		if (skipJob.indexOf(jobUUID) != -1){continue}
		if (!Q.minorid){continue}
		var grade = (Q.bomgrade) ? Q.bomgrade.trim() : '';
		var finish = (Q.bomfinish) ? Q.bomfinish.trim() : '';
		var heatNum = (Q.bomheat) ? Q.bomheat.trim() : '';
		var heatId = globals.createHeat(event,heatNum);
		var lotNum = (Q.bomlot) ? Q.bomlot.trim() : '';
		scopes.jobs.importJob.jobId = jobUUID;
		var lotId = scopes.jobs.createLotNumber(lotNum);
		application.output('Heat: '+heatNum+' Id:'+heatId);
		var seqNum = (Q.bomseqnum) ? Q.bomseqnum.trim() : '';
		var seqId = scopes.jobs.createSequenceNumber(seqNum);
		//var statusID = getStatusDescriptionCO(event,Q.idstatus);//MISSING
		//application.output('Status: '+Q.idstatus+' id: '+statusID);
		var serialID = createSerialIdCO(event,Q.minorid,jobUUID);
		application.output('id '+Q.minorid+' UUID: '+serialID);
		//MISSING origempl application.output('emp: '+Q.origempl+' '+getEmployeeIDCO(event,Q.origempl))
		var inventoryID = (Q.bomrawid) ? globals.getInvUUID(event,Q.bomrawid) : null;
		var started = true;
		if (started){
			// piecemark is major, minor, sheet, grade, finish - makes it unique
			/** @type {QBSelect<db:/stsservoy/piecemarks>} */
		...
		// xlots, xloads, xstatus_description, idserialID, xheats, raw_material, guids, 
		if (R.getSize() == 0){
			application.output('skipping: '+Q.parentPcmk+' '+Q.bompartnum+' '+' '+Q.jobUUID);
			continue;
		} else {
			newRecCount++;
			indexCount++;
			if (started){
				if (!sheetUUID){
					application.output(Q.jobid+' '+Q.parentPcmk+' '+Q.bompartnum+' '+Q.sheetnum+' '+sheetUUID+' ');
				}
				/** @type {QBSelect<db:/stsservoy/idfiles>} */
				var s = databaseManager.createSelect('db:/stsservoy/idfiles');
				var S = databaseManager.getFoundSet(s);
				...
				rec.sequence_id = globals.makeUUID(seqId);//Q.seqnum;
				rec.lot_id = globals.makeUUID(lotId);//Q.lot.trim();
			//MISSING rec.id_location = (Q.idlocation) ? Q.idlocation.trim() : '';
			//rec.status_description_id= statusID;//idstatus/fabshop
				rec.id_serial_number_id = serialID;//id
				rec.deleted_date = (Q.delete_dt) ? globals.dateAndTimeToDate(Q.delete_dt,null) : null;//delete_dt
			//MISSING rec.mrr = (Q.mrr) ? Q.mrr.trim() : '';//mrr
				rec.heat_id = heatId;//heat
			//MISSING rec.fabrication_invoice = (Q.fabinvoice) ? Q.fabinvoice.trim() : '';//fabinvoice
			//MISSING rec.id_area = (Q.area) ? Q.area.trim() : '';//area
			//MISSING rec.revision_level = (Q.revlevel) ? Q.revlevel.trim() : '';//revlevel
			//UNKNOWN rec.id_guid = (Q.guid) ? Q.guid.trim() : '';//guid
				rec.id_finish = (Q.mobfinish) ? Q.mobfinish.trim() : '';//idfinish
			//MISSING rec.id_batch = (Q.idbatch) ? Q.idbatch.trim() : '';//idbatch
				rec.raw_material_id = inventoryID;//rawmatlid
				
				rec.guid_minor = (Q.guid) ? Q.guid : null;//guid or bomguid
				//labelqty how many labels
				//labelbc how many on a label
			//MISSING origempl rec.original_employee_uuid = getEmployeeIDCO(event,Q.origempl);//origempl
				rec.current_load_id = getLoadIDCO(event,Q.loadnum,jobUUID);//loadnum
			//MISSING rec.ship_load_id = getLoadIDCO(event,Q.shipload,jobUUID);//shipload
			//MISSING rec.received_load_id = getLoadIDCO(event,Q.recvload,jobUUID);//recvload
				
				rec.summed_quantity = 1;
				rec.original_quantity = Q.origqty;//pcmkcount
			//UNKNOWN rec.id_guid;//minorguid
			//MISSING rec.off_hold_date = (Q.offholddate) ? globals.dateAndTimeToDate(Q.offholddate,null) : null;//offholddate
			//MISSING rec.cost_date = (Q.costdate) ? globals.dateAndTimeToDate(Q.costdate,null) : null;;//costdate
			//MISSING rec.ut_test = Q.uttst;//uttst
			//MISSING rec.sds_member = Q.sts_member;//sds_member
				
			//MISSING rec.shipped_quantity = (Q.shipqty) ? Q.shipqty : null;//shipqty
			//MISSING rec.receive_quantity = (Q.recvqty) ? Q.recvqty : null;//recvqty
			//MISSING rec.object_id = (Q.objectid) ? Q.objectid.trim() : '';//objectid
			//MISSING rec.longitude = (Q.longitude) ? Q.longitude.trim() : '';//longitude
			//MISSING rec.latitude = (Q.latitude) ? Q.latitude.trim() : '';//latitude
			//MISSING rec.altitude = (Q.altitude) ? Q.altitude.trim() : '';//altitude
			//MISSING rec.erection_drawing = (Q.erectdwg) ? Q.erectdwg.trim() : '';;//erectdwg
			//MISSING rec.create_date = (Q.idcreated) ? globals.dateAndTimeToDate(Q.idcreated,null) : null;//idcreated
			//MISSING rec.required_ship_date = (Q.reqdship) ? globals.dateAndTimeToDate(Q.reqdship,null) : null;//reqdship
			//MISSING rec.scheduled_ship_date = (Q.schdship) ? globals.dateAndTimeToDate(Q.schdship,null) : null;//schdship
			//MISSING rec.reference_date = (Q.refdate) ? globals.dateAndTimeToDate(Q.refdate,null) : null;//refdate
			//MISSING rec.on_hold_date = (Q.onholddate) ? globals.dateAndTimeToDate(Q.onholddate,null) : null;//onholddate
				rec.lprint = (Q.lprint) ? true : null;//lprint
			//MISSING rec.pmi_test = (Q.pmitest) ? Q.pmitest : null;//pmitest
			//MISSING rec.stress_test = (Q.stresstst) ? Q.stresstst : null;//stresstst
			//MISSING rec.bhn_test = (Q.bhntest) ? Q.bhntest : null;//bhntest
			//MISSING rec.hydro_test = (Q.hydrotst) ? Q.hydrotst : null;//hydrotst
			//MISSING rec.qc_final_test = (Q.qcfinaltst) ? Q.qcfinaltst : null;//qcfinaltst
			//MISSING rec.ft_partid = Q.ft_partid;//ft_partid
			//MISSING rec.ft_assyid = Q.ft_assyid;//ft_assyid
			//MISSING rec.ft_assinid = Q.ft_assinid;//ft_assinid
			//MISSING rec.ft_lot = Q.ft_lot;//ft_lot
			//MISSING rec.ft_batch = Q.ft_batch;//ft_batch
			//MISSING rec.ft_seqid = Q.ft_seqid;//ft_seqid
			//MISSING rec.ft_sitid = Q.ft_sitid;//ft_sitid
			//MISSING rec.ft_pkgid = Q.ft_pkgid;//ft_pkgid
			//MISSING rec.ft_pkgno = Q.ft_pkgno;//ft_pkgno
				rec.bundle_bc = (Q.bundleid) ? Q.bundleid.trim() : null;//bundleid
			//MISSING rec.id_on_hold = Q.idonhold;//idonhold
				rec.print_date = (Q.print_dt) ? globals.dateAndTimeToDate(Q.print_dt,null) : null;//print_dt
			//UNUSED recaction
				//recList.push(rec);
				// origqty = qty on this serial number, labelqty = number of barcodes used this pcmk, labelbc=total pcs on this barcode
				var guidArray = [];var guidFS = '';
				if (itemCount > 1){
					// collect guid for main mark in an array, then pop into each copy of the idfile
					/** @type {QBSelect<db:/dbf_data/id_file_multiguid>} */
					var b = databaseManager.createSelect('db:/dbf_data/id_file_multiguid');
					b.where.add(b.columns.jobid.eq(Q.jobid));
					b.where.add(b.columns.parentid.eq(Q.minorid));
					b.result.add(b.columns.multiguid);
					var B = databaseManager.getDataSetByQuery(b,-1);
					guidArray = B.getColumnAsArray(1);
					rec.guid_minor = guidArray.pop();
